...
try:
    ...
    assert 'app.add_middleware(RateLimitMiddleware)' in app_content, \
...
try:
    ...
    assert 'from .rate_limit import update_quota_in_db' in middleware_content, \
...
try:
    ...
    assert 'print(f"API Key (send to user): {raw_key}")' not in whop_content, \
...
try:
    ...
    assert "os.getenv('ENVIRONMENT', 'development') == 'production'" in whop_content, \
