...
def main():
    ...
    success, output = run_command(['python3', '-m', 'unittest', 'tests.test_memory_manager', '-v'], timeout=30)
    if success:
        lines = output.split('\n')
    ...
    success, output = run_command(['python3', '-m', 'unittest', 'tests.test_mcp_integration', '-v'], timeout=30)
    if success:
        lines = output.split('\n')
        summary = [l for l in lines if 'Ran' in l or 'OK' in l or 'FAILED' in l]
