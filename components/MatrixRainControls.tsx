...
const CONTROLS: ControlDef[] = [
  ...
  { key: "columnStride", label: "Column spacing", min: 12, max: 48, step: 1 },
  { key: "trailSteps", label: "Trail length", min: 0, max: 24, step: 1 },
  { key: "dropStep", label: "Drop speed", min: 1, max: 20, step: 1 },
  { key: "glimmerChance", label: "Glimmer %", min: 0, max: 0.2, step: 0.001 },
  { key: "freezeChance", label: "Freeze %", min: 0, max: 0.05, step: 0.001 },
  { key: "charChangeInterval", label: "Char interval", min: 1, max: 20, step: 1 },
  { key: "lingerFrames", label: "Linger frames", min: 0, max: 20, step: 1 },
  { key: "fadeMode", label: "Fade mode", min: 0, max: 1, step: 1 },
];
...
export function MatrixRainControls() {
  ...
  // Toggle panel visibility with the rain toggle button + settings gear
  // Always accessible — no unlock required
  const togglePanel = useCallback(() => {
    setVisible((v) => !v);
  }, []);

  // Expose toggle globally so the MatrixRain button can open controls
  useEffect(() => {
    (window as unknown as { __wmRainControlsToggle?: () => void }).__wmRainControlsToggle = togglePanel;
    return () => {
      delete (window as unknown as { __wmRainControlsToggle?: () => void }).__wmRainControlsToggle;
    };
  }, [togglePanel]);
  ...
  return (
    <div className="neo-panel neo-panel-enter fixed bottom-16 right-4 z-50 w-72 rounded-xl border border-border bg-surface/90 p-4 shadow-lg backdrop-blur-sm">
      <div className="mb-3 flex items-center justify-between">
        ...
        <button
          ...
          className="font-mono text-[10px] text-dim hover:text-fg"
        >
          close ✕
        </button>
      </div>
          ...
          <div key={def.key} className="flex flex-col gap-1">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-[10px] text-lavender">
                {def.key === "fadeMode"
                  ? neoStore.matrixRain[def.key] > 0.5 ? "trail" : "clear"
