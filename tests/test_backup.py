...
class TestBackupManager:
    ...
    def test_list_backups(self, tmp_path):
        ...
        assert (
            len(backups) >= 1
        ), f"Expected at least 1 backup, got {len(backups)}. Backup dir: {backup_mgr.backup_dir}, files: {list(backup_mgr.backup_dir.glob('*'))}"
    ...
    def test_restore_backup_dry_run(self, tmp_path):
        ...
        # Dry run restore
        restore_result = backup_mgr.restore_backup(backup_path, dry_run=True)
    ...
    def test_restore_backup_full(self, tmp_path):
        ...
        restore_result = backup_mgr.restore_backup(
            backup_path, target_dir=restore_dir, dry_run=False
        )
