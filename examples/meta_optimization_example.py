...
from whitemagic import (  # Meta-optimization imports
    configure_session,
...
def example_1_workspace_loading():
    ...
    context_tier0 = load_workspace_for_task(
        str(workspace_root), "Implement symbolic reasoning module", tier=0
    )
    print("\nTier 0 (Minimal):")
    ...
    context_tier1 = load_workspace_for_task(
        str(workspace_root), "Implement symbolic reasoning module", tier=1
    )
    print("\nTier 1 (Balanced):")
...
def example_2_smart_start_here():
    ...
    print("   Estimated: <1K tokens for next session resume")
    ...
    print("   Estimated: ~3K tokens for next session resume")
...
def example_5_full_workflow():
    ...
    config = configure_session(
        task_description="Continue implementing v2.2.5 features", memory_dir=Path("memory")
    )
    ...
    print("  Tracking: features, decisions, insights, metrics")
    ...
    print("  Next session will load in <1K tokens")
    ...
    print("  Traditional approach: ~20,000 tokens")
