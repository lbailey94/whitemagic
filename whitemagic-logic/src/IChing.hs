...
fromNumber n 
  ...
  where
    ...
    numToHex num =
        let bits = reverse $ take 6 $ toBinary num ++ repeat 0
            [l1, l2, l3, u1, u2, u3] = map bitToLine bits
        in hexagram l1 l2 l3 u1 u2 u3
...
transition :: [Int]  -- ^ Line positions to change (1-6, bottom to top)
           -> Hexagram
           -> Hexagram
transition positions hex@(Hexagram (Trigram u1 u2 u3) (Trigram l1 l2 l3)) =
    let lines = [l1, l2, l3, u1, u2, u3]
        changed = zipWith (\i line -> if i `elem` positions 
                                       then oppositeLine line 
                                       else line) [1..6] lines
        [nl1, nl2, nl3, nu1, nu2, nu3] = changed
    in hexagram nl1 nl2 nl3 nu1 nu2 nu3
...
yinYangRatio (Hexagram (Trigram u1 u2 u3) (Trigram l1 l2 l3)) =
    let lines = [l1, l2, l3, u1, u2, u3]
        yinCount = length $ filter (== Yin) lines
