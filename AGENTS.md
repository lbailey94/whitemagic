...
- v24.2.0: MandalaOS Phase A-D — Karmic effect types (`EffectType` enum, `EffectSignature` dataclass, effect registry auto-inferring signatures for 692 tools); `mw_karma_effects` middleware auto-recording declared/actual effects for every tool call; 3 karmic MCP tools (`karmic.effects`, `karmic.debt`, `karmic.verify`); Mandala compartments (`dharma_profile` + `template` on `Shelter`, `SHELTER_TEMPLATES` with research/sandbox/production/secure, `shelter_id` on `KarmaEntry`); 4 mandala MCP tools (`mandala.create/status/destroy/templates`); Koka effect enforcement (`karmic_effects.kk`, `KokaNativeBridge.dispatch_karmic()`, `HybridDispatcher.karmic_compare()` with Python fallback); 2 effect visualization MCP tools (`effect.trace`, `effect.visualize` with DOT/Mermaid/JSON); `/mandala` Next.js dashboard page with live widgets; MC simulation integration (Bayesian optimization, rare event simulation, SDE solvers via `polyglot_mc.py` + `evolution_bridge.rs`; `SimulationOrchestrator` wired into `RecursiveImprovementLoop` prediction phase; superforecaster pipeline + `superforecaster_deep_optimization` autoswarm campaign); Local model wiring completed — `LlamaCppBackend.chat()` Qwen3 reasoning_content fallback; speculative decoding E2E verified with Qwen3-1.7B (draft) + Qwen3-4B (verify), tokenizer compatibility confirmed, 31% acceptance rate; ngram-mod speculative decoding tested on single model; 10 new speculative decoding + LlamaCppBackend tests; 41 MandalaOS tests + 10 inference tests, 379 existing dispatch/middleware/pipeline/registry tests all passing
- v24.3.0: Screenshot Upgrade Strategy — 7 new features + 17 new MCP tools (729 total dispatch entries); Transaction Firewall (`security/transaction_firewall.py`, `mw_transaction_firewall` middleware — per-agent spend limits, rate limiting, recipient allowlist/blocklist, Dharma ethical sign-off, audit log); BountyPlatform Auto-Connector (`agents/bounty_connector.py` — external bounty scanning, capability-based agent matching, auto-claim + import to local BountyBoard, ReachingAI + Mock platform adapters); Model Auto-Optimization Loop (`inference/auto_optimizer.py` — benchmark → explore → converge loop, parameter space search, config persistence, fitness = tokens/s × quality / memory); Ambient Sensorium Layer (`core/consciousness/ambient_sensorium.py` — SystemPressure/UserPattern/Temporal/Environment sensors, ambient state aggregation, proactive action suggestions, background polling thread); WASM Compute Verification (`security/wasm_verifier.py`, `mw_wasm_verify` middleware — checksum + replay verification for pure/read tools, karmic debt on mismatch, opt-in via WM_WASM_VERIFY=1); Network State Profile (`core/identity/network_state.py` — sovereign agent identities with Ed25519 keypairs, reputation tracking, governance proposals with reputation-weighted voting, stake accumulation); Genetic Algorithm Harness (`core/evolution/genetic_harness.py` — tournament selection, uniform crossover, bounded Gaussian mutation, elitism, convergence detection); 96 new unit tests + 9 integration tests, 514 existing related tests passing, zero regressions; Cache Coherence & Hot-Path Optimization Strategy — 12 features across 4 tiers (version-aware HybridRecallCache, Markov persistence, HomeostaticLoop cache health, `cache.tune` MCP tool, Redis pub/sub cross-process invalidation, idle cache warming via consciousness loop, embedding-based semantic cache keys with cosine similarity, CRDT LWW merge for multi-agent memory, cache-aware dispatch routing with governance skip, citta-informed speculative prefetch, holographic spatial invalidation); deeper integrations (citta prediction in `on_call_complete`, spatial invalidation in `store()`/`update_memory()`, `merge_remote_memory()` in galaxy sync); 24 new tests, 5247 total unit tests passing, 0 regressions; MC simulation handler fixes — 9 missing handlers in `simulation.py` (mc.surrogate, mc.optimize, mc.rare_event, mc.sde, mc.superforecaster, simulation.introspect/forecast/status/recursive) with correct API signatures for `PolyglotMCOrchestrator` + `SimulationOrchestrator`; P5 integration — DreamCycle delegation in `dream_integration.py` (lazy import, fallback to simulated phases), EmergenceEngine novelty filtering in `insight_synthesizer.py` (confidence field + `_filter_via_emergence_engine`), PolyglotMCOrchestrator Rust-accelerated LHS in `scenario_runner.py` (5000+ trial threshold); 19 v24.3.0 PRAT mappings added (ambient, bounty, genetic, model, network_state, tx_firewall); benchmark campaign 780 tools — 95.6% adjusted rate (600 success + 101 expected + 11 unexpected + 21 timeout), PASS; DB migration audit complete — 0 raw `sqlite3.connect()` in production code, all migrated to `safe_connect()`
- v24.3.1: Memory System Improvements — CurrentStateTracker (`core/memory/current_state.py` — live work state snapshot with current task, active tasks, next steps, recent file modifications, decisions, errors, context; auto-persists to `state/current_state.json` + sessions galaxy memory; injected into MCP server instructions on connect); 3 new MCP tools (`state.current`, `state.update`, `state.context`) mapped to `gana_heart` with full dispatch + PRAT + registry wiring (756 total dispatch entries); Search result enrichment — `handle_search_memories` now returns `title`, `galaxy`, `tags`, `importance` fields (was just `id` + 200-char content); `full_content` option increases preview from 200→500 chars; Session recorder middleware improvements — records meaningful content from tool args (`content`, `query`, `description`, etc.) instead of just `"Tool: {name} → {status}"`; auto-tracks file modifications and errors in CurrentStateTracker; WorkingMemory persistence — `save_to_disk()`/`load_from_disk()` methods, auto-loads on singleton creation, auto-saves on session end (cross-session attentional focus continuity); SessionHandoff.end_session() auto-wiring — pushes handoff data to CurrentStateTracker, auto-triggers sleep consolidation (important turns → codex galaxy), auto-saves WorkingMemory; MCP instructions updated — `state.current` as first quick-start step and first agent onboarding step; 29 new tests, 61 existing memory tests, 51 existing session tests all passing
- v24.3.2: Cognitive Action Flywheel Phases 3-5 — SignalWeightTracker (EMA-based per-source weight adjustment from outcome deltas, persisted to `signal_weights` table); Cross-session outcome analysis in `get_status()` (per-action success rates, impact rankings); Emergence insight deduplication (content_hash + source/title/description pre-check before insert, `INSERT OR IGNORE`); Pattern applicability matching (`_match_patterns_to_signals` — keyword overlap search attaching `applicable_patterns` to signal metadata); Citta → action loop trigger (`_check_ignition_trigger` — 5+ ignitions in 20 moments triggers cycle); Dream → emergence feedback (pre/post-dream insight count comparison in `trigger_cycle`); Knowledge gap → action signal (KnowledgeGapActionLoop results wired into `_collect_signals`); RIL → action loop execution (`_execute_auto_fixable` — auto-fixable proposals executed via action loop); Multi-objective Pareto optimization (5D scoring: urgency 30%, impact 25%, novelty 20%, cost 10%, learning 15%); Speculative action execution (idle-time background thread for low-cost actions); Action outcome prediction (per-action positive rate tracking, skip if <20% after 5+ observations); Cross-agent action coordination (per-action `threading.Lock` with non-blocking acquire); Consciousness-driven action selection (citta depth/velocity/ignition adjusts signal urgency); Strategy doc archived to `docs/completed/`
