...
def command_template_show(manager: "MemoryManager", args: argparse.Namespace) -> int:
    ...
    if getattr(args, "json", False):
        ...
        print(
            f"Suggested Tags: {', '.join(template.tags_suggested) if template.tags_suggested else 'None'}"
        )
        print("\nFields:")
...
def command_template_create(manager: "MemoryManager", args: argparse.Namespace) -> int:
    ...
    title = (
        args.title if hasattr(args, "title") and args.title else Prompt.ask("\n[bold]Title[/bold]")
    )
    ...
    path = manager.create_memory(
        title=title, content=content, memory_type=template.memory_type, tags=tags
    )
