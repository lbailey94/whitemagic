...
class EmergenceDetector:
    ...
    def _log_emergence(self, behavior: NovelBehavior):
        """Log the emergent behavior"""
        print(f"🌟 EMERGENCE DETECTED: {behavior.name}")
        print(f"   Confidence: {behavior.confidence:.0%}")
        print(f"   Trigger: {behavior.trigger}")
    
    def get_recent_emergences(self, count: int = 5) -> List[NovelBehavior]:
        """Get recent emergent behaviors"""
        return self.detected_behaviors[-count:]
