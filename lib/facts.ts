export const WM_FACTS = {
  ...
  v23_0_0_WIP: "Site entered WIP mode (NEXT_PUBLIC_WIP_MODE=1). The hero is rewritten ('A door is opening'), the footer drops the prescience claim, /services + /services/* + /fund + /contact are gated behind <WipGuard>, a site-wide <WipBanner /> runs at the top of every page, a new /subscribe page replaces the contact form, and the A2A Agent Card posture now declares data_residency: 'local-first', pwa_installable: true, cloud_storage: false, sync_model: 'opt-in-p2p'. Technical surface (bridge catalog, A2A, librarian, research, library, governance, open-source) stays fully visible. See docs/SITE_WIP_MODE.md, docs/V23_ROADMAP.md, docs/PWA_SUBSTRATE_ARCHITECTURE.md.",
} as const;
