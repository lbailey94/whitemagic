...
## Priority 0: Interactive Setup Wizard (HIGH)
...
│ Welcome to WhiteMagic! Let's get you set up.      │
│                                                    │
│ Current install: Minimal (~50MB)                  │
...
│   [ ] Local AI & Semantic Search (+2.5GB)        │
│       Privacy-first, works offline               │
│   [ ] Nothing - I'll use OpenAI API              │
│                                                    │
│ [Continue]                                         │
╰────────────────────────────────────────────────────╯
```
...
1. **Embeddings**: Local (privacy) or OpenAI (lightweight)?
2. **Model Download**: Install now or later?
3. **Configuration**: Default paths or custom?
4. **API Key** (if OpenAI): Set now or skip?

**Output**:
```bash
✓ Configuration saved to ~/.whitemagic/config.yaml
✓ Installing extras: whitemagic[embeddings]
✓ Downloading model: all-MiniLM-L6-v2 (88MB)
✓ Setup complete!
    ...
    def run(self):
        ...
        if not current.has_embeddings:
            ...
            if choice == "local":
                ...
                self.save_config({"embeddings": {"provider": "openai"}})
