...
results = parallel_read([
    grep_search("v2.2.3", docs/),
    grep_search("missing features", roadmap/),
    read_file(vision.md),
    read_file(v2.2.1_plan.md),
])  # 30K tokens
...
cache.set("session/v2.2.3_context", summary)
...
| **1** | Balanced | 5-15K | "Find missing features in v2.2.3" |
...
2. **Test with v2.2.3 work** (real-world validation)
