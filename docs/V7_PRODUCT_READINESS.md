...
| G1.4 Linux x86-64 install | Pending | Clean-account transcript, checksum, version, doctor, MCP handshake |
| G1.5 Product quickstart | Pending | Two-process continuity demo on isolated store |
| G1.6 Agent instructions | Pending | Supported-client behavior test |
| G1.7 Continuity process gate | Pending | CI and artifact-level pass |
| G1.8 Documentation truth | Pending | Command/link/count/claims audit |
| G1.9 Backup/restore | Pending | Full-state disaster-recovery process pass |
