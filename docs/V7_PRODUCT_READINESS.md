...
| Static musl Linux build | **Complete** (2026-08-26) | `x86_64-unknown-linux-musl` added to release matrix (`wm-linux-x86_64-musl`); local build verified fully static-pie and passed the full curated smoke test (continuity + backup gates); CI renames before hashing so checksums reference distributed names. Version line corrected to `7.0.0-alpha.4` (G1.4 follow-up: alpha.2 binaries had reported alpha.1). Installer prefers the static build and refuses old dynamic builds on glibc < 2.39 with an actionable message; fallback path live-tested against v7.0.0-alpha.3 from a clean temp dir. |
| Multi-laptop workflow | **Documented** (2026-08-26) | `docs/MULTI_LAPTOP.md`: whole-store move via `wm backup`/`wm restore` (last-restore-wins semantics stated), lightweight per-session carry via `session.export`/`session.import`, install + doctor verification steps, single-writer and backup hygiene rules. Linked from README docs list. |
| Retired "Latest" badge | **Resolved by removal** (2026-08-26) | GitHub cannot flag a prerelease as Latest, so v5.8.0 kept the false Latest marker. Both retired releases (v5.8.0, v5.8.0-rc1; ids 374051134/370280470) were first flagged prerelease — but that left `/releases/latest` redirecting to a prerelease and kept retired binaries downloadable (against G1.3). Both release objects were then deleted; git tags `v5.8.0`, `v5.8.0-rc1` remain for history. `/releases/latest` now redirects to the releases page itself. Restore path if ever needed: recreate a release from the existing tags. |
| v7.0.0-alpha.4 release | **Cut** (2026-08-26) | Tag `v7.0.0-alpha.4` → CI built 10 assets incl. `wm-linux-x86_64-musl`; Linux musl + gnu builds smoke-tested in CI; marked prerelease with truthful notes. End-to-end verified live: unpinned installer resolves alpha.4, prefers the static build, checksum verifies against distributed names, `wm --version` prints `wm 7.0.0-alpha.4` matching its page. Note: first tag push failed to start Release at all (workflow YAML indentation fault, commit `5fad063` fixed it); tag was deleted and re-cut on the fix — no partial release object existed between pushes. |
| v7.0.0-alpha.5 release | **Cut** (2026-08-29) | Fresh public history force-pushed (`b5d4b6b`, full pre-public history preserved locally); old tags/releases deleted; push-protection caught the detector's own Slack fixture → runtime-assembly fix (`493ca5a`); archive trimmed to bare-minimum v7 (`69eb5c6`); all 5 platform assets published and marked Latest; first CI failure fixed (`44423d6`). |
| v7.0.0-alpha.6 release | **Cut** (2026-08-29, evening) | R0 mesh slice (`3edf0d4`), clippy sweep to 0 warnings (`73e7e0f`), provenance-attribution fix (`68547b9`), memory.query substring filter (`6da07b4`), rehearsal runbook (`2c306aa`). Pre-push secrets scan clean; Release workflow green first try (5 platform builds + Publish); marked Latest; published `wm-linux-x86_64` artifact smoke-verified end to end (sha256, `--version`, quickstart, create→search round-trip, doctor all-OK). Same evening: batched fleet rebuild — landlock git-dir grant proven live through the confined wmv5 unit (`code.claim`/`release` full lifecycle), gateway contract ok, mesh off-by-default verified via `/status`. |
