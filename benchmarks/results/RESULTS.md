...
| 2026-09-04 | **`v8_sliceb_baseline`** | **0.86** | **1.00** | **0.923** | Slice B ship gate: DEFAULT `memory.search` route on the Slice B tree (validity field + `validity_sweep` + third visibility predicate, all knobs OFF) — **byte-identical to baseline (sixth consecutive 0.86/1.00/0.923)**. Debug binary `target/debug/wm` (quality-identical, latency not comparable: p50 232ms / p95 554ms search-only). Per-query records in `longmemeval_s_v8_sliceb_baseline.json`. |
