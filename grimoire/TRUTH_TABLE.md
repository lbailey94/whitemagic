...
## Extended Tool Assignments (v23.0.0)

The 28-fold mapping above lists the canonical "Real Tools" per Gana from the original
v22.0.0 table. The dispatch table has since grown to 462 tools, many of which were
added without explicit Gana assignment. This section assigns **all 387 previously-
orphaned tools** to their Gana/Garden homes based on semantic domain alignment.

### Ch 1 Horn — Session Initiation (courage)
`session_bootstrap`, `create_session`, `resume_session`, `checkpoint_session`, `focus_session`, `session_status`, `session.handoff`, `session.handoff_transfer`, `session.list_handoffs`, `session.accept_handoff`, `session_handoff_summary`

### Ch 2 Neck — Memory CRUD (stillness)
`create_memory`, `update_memory`, `import_memories`, `delete_memory`, `remember`, `thought_clone`, `memory_delete`, `memory_update`

### Ch 3 Root — System Health (healing)
`health_report`, `rust_status`, `rust_similarity`, `rust_audit`, `rust_compress`, `ship.check`, `state.paths`, `state.summary`, `check`, `pulse.status`

### Ch 4 Room — Security & Sandbox (sanctuary)
`sangha_lock`, `sangha_lock_acquire`, `sangha_lock_list`, `sangha_lock_release`, `sandbox.set_limits`, `sandbox.status`, `sandbox.violations`, `security.alerts`, `security.monitor_status`, `mcp_integrity.snapshot`, `mcp_integrity.status`, `mcp_integrity.verify`, `hermit.assess`, `hermit.check_access`, `hermit.mediate`, `hermit.resolve`, `hermit.status`, `hermit.verify_ledger`, `hermit.withdraw`, `immune_scan`, `immune_heal`, `anti_loop_check`

### Ch 5 Heart — Context (love)
`scratchpad`, `scratchpad_create`, `scratchpad_update`, `scratchpad_finalize`, `analyze_scratchpad`, `context.pack`, `context.status`, `get_session_context`, `working_memory.attend`, `working_memory.context`, `working_memory.status`

### Ch 6 Tail — Performance (courage)
...
### Ch 10 Star — Governance (voice)
`governor_validate`, `governor_set_goal`, `governor_check_budget`, `governor_check_dharma`, `governor_check_drift`, `governor_stats`, `governor_validate_path`, `dharma.reload`, `dharma_rules`, `set_dharma_profile`, `forge.status`, `forge.reload`, `forge.validate`, `guideline.evolve`

### Ch 11 Extended Net — Patterns (sangha)
`pattern_search`, `cluster_stats`, `tool.graph`, `tool.graph_full`, `learning.patterns`, `learning.suggest`, `learning.status`, `association.mine`, `association.mine_semantic`, `causal.mine`, `causal.stats`, `community.health`, `community.propagate`, `community.status`, `constellation.detect`, `constellation.merge`, `constellation.stats`, `emergence.scan`, `emergence.status`, `novelty.detect`, `novelty.stats`, `pattern_consciousness.status`, `resonance_trace`, `coherence_boost`, `entity_resolve`
...
### Ch 15 Straddling Legs — Ethics (awe)
`evaluate_ethics`, `check_boundaries`, `verify_consent`, `harmony_vector`, `get_dharma_guidance`, `get_ethical_score`, `karma_record`, `karma_report`, `karmic_trace`, `karma.anchor`, `karma.anchor_status`, `karma.verify_anchor`, `karma.verify_chain`, `verification.attest`, `verification.request`, `verification.status`, `wu_xing_balance`

### Ch 16 Mound — Metrics (gratitude)
`view_hologram`, `track_metric`, `get_metrics_summary`, `record_yin_yang_activity`, `get_yin_yang_balance`, `cache.flush`, `cache.status`, `green.record`, `green.report`

### Ch 17 Stomach — Tasks (creation)
`pipeline`, `pipeline.create`, `pipeline.list`, `pipeline.status`, `task.complete`, `task.distribute`, `task.list`, `task.route_smart`, `task.status`

### Ch 18 Hairy Head — Debug (presence)
`salience.spotlight`, `anomaly`, `anomaly.check`, `anomaly.history`, `anomaly.status`, `otel`, `otel.metrics`, `otel.spans`, `otel.status`, `voice_audit.scan`, `voice_audit.status`, `voice_audit.quarantine_list`, `watcher_add`, `watcher_list`, `watcher_remove`, `watcher_start`, `watcher_stop`, `watcher_status`, `watcher_stats`, `watcher_recent_events`

### Ch 19 Net — Capture (play)
`prompt.render`, `prompt.list`, `prompt.reload`

### Ch 20 Turtle Beak — Precision (practice)
`edge_infer`, `edge_batch_infer`, `edge_stats`, `edge_add_rule`, `bitnet_infer`, `bitnet_status`

### Ch 21 Three Stars — Judgment (reverence)
`reasoning.bicameral`, `reasoning.multispectral`, `ensemble`, `ensemble.history`, `ensemble.query`, `ensemble.status`, `solve_optimization`, `kaizen_analyze`, `kaizen_apply_fixes`, `art_of_war.assess`, `art_of_war.campaign`, `art_of_war.chapter`, `art_of_war.plan`, `art_of_war.terrain`, `art_of_war.wisdom`, `think`, `consult_wisdom_council`, `sabha.convene`, `sabha.status`, `corpus_callosum.debate`, `corpus_callosum.status`, `foresight.analyze`, `foresight.constellations`, `foresight.convergence`, `foresight.decay`, `satkona.fuse`, `elemental.optimize`

### Ch 22 Dipper — Strategy (dharma)
`homeostasis`, `homeostasis.check`, `homeostasis.status`, `maturity.assess`, `starter_packs`, `starter_packs.get`, `starter_packs.list`, `starter_packs.suggest`, `astro_status`, `astro_shift`, `cognitive.mode`, `cognitive.set`, `cognitive.hints`, `cognitive.stats`, `neurotransmitter.report`, `neurotransmitter.status`, `doctrine.force`, `doctrine.stratagems`, `doctrine.summary`

### Ch 23 Ox — Endurance (patience)
`swarm.complete`, `swarm.decompose`, `swarm.plan`, `swarm.resolve`, `swarm.route`, `swarm.status`, `swarm.vote`, `war_room.campaigns`, `war_room.execute`, `war_room.hierarchy`, `war_room.phase`, `war_room.plan`, `war_room.status`, `worker.status`

### Ch 24 Girl — Nurture (connection)
`agent.register`, `agent.heartbeat`, `agent.list`, `agent.capabilities`, `agent.deregister`, `agent.trust`

### Ch 25 Void — Gardens & Galaxies (mystery)
