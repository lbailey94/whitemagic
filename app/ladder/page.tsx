  ...
  return (
    <>
      <PageHeader
        ...
        lede="Eight tiers, from casual chatbot user to sovereign AI operator. Click your rung to see what's possible at your level — and the exact workshop that gets you to the next one."
      />
      ...
      <section className="container-site py-16">
        <div className="mx-auto max-w-3xl space-y-5 text-muted">
          <p className="leading-relaxed">
            ...
            <em className="text-fg">&ldquo;I use ChatGPT sometimes&rdquo;</em>{" "}
            (Tier 0) or{" "}
            <em className="text-fg">
              &ldquo;I&rsquo;m building agents&rdquo;
            </em>{" "}
            (somewhere between Tiers 3 and 6, usually overstated). The ladder
            below is the vocabulary I use in workshops to get past that. Each
            tier describes a real capability plateau: what it looks like for
            <em> you</em>, what you can actually do, and the specific workshop
            that bridges to the next rung.
          </p>
          <p className="leading-relaxed">
            Two caveats. First, you can be at different tiers for different
            tasks — Tier 1 for writing emails, Tier 3 for coding, Tier 0 for
            bookkeeping. The interesting question is the gap, not the average.
            Second, climbing isn&rsquo;t the goal. For most small business
            owners, Tier 2 is the destination. For developers building
            products, Tier 4 is where life gets interesting. Match the rung
            to the work.
          </p>
          <p className="leading-relaxed">
        ...
        <div className="mx-auto max-w-4xl space-y-6">
          {TIERS.map((tier) => (
            <TierRow
              key={tier.num}
              tier={tier}
              isSelected={selectedTier === tier.num}
              onSelect={() =>
                setSelectedTier(selectedTier === tier.num ? null : tier.num)
              }
            />
          ))}
        </div>
      ...
      {selectedTier !== null && (
        <section className="container-site pb-24">
          <NextStepPanel tier={TIERS[selectedTier]} />
        </section>
      )}
      ...
      <section className="container-site py-20 text-center">
        ...
        <p className="mx-auto mb-6 max-w-xl text-muted">
          Book a free 15-minute call. Bring your current AI setup (or the
          fact that you don&apos;t have one). Leave with a clear read on your
          tier and the exact workshop or session that gets you to the next
          rung.
        </p>
...
function TierRow({
  ...
  return (
    <article
      onClick={onSelect}
      className={`cursor-pointer rounded-2xl border p-6 transition md:p-8 ${
        isSelected
          ? "border-lavender bg-lavender-bg shadow-sm"
          : "border-border-light bg-surface hover:border-lavender-light"
      }`}
      id={`tier-${tier.num}`}
    >
      {isSelected && (
        <div className="mb-3 flex items-center gap-2 text-lavender">
      ...
      <div className="mb-5 flex items-start gap-4">
        <div className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-xl text-lavender ${isSelected ? "bg-white/50" : "bg-surface-alt"}`}>
...
function NextStepPanel({ tier }: { tier: Tier }) {
  ...
  const workshop = WORKSHOPS[tier.num];

  return (
    <div className="rounded-2xl border border-lavender bg-lavender-bg p-6 md:p-8">
      ...
      {workshop ? (
        <div className="grid gap-6 md:grid-cols-[1fr_auto]">
          ...
          <div className="flex items-start md:justify-end">
            <Link href="/workshops" className="btn-primary whitespace-nowrap">
              {workshop.cta}
              <ArrowRight className="ml-2 h-4 w-4" />
            </Link>
          </div>
        ...
        <div>
          ...
          <p className="text-muted">
            Tier 7 operators usually need something custom. If you want to talk
            shop, book a call — no pitch, just peers.
          </p>
          <div className="mt-4">
            <Link href="/contact" className="btn-ghost">
              Book a call
