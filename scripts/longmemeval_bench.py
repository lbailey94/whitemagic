...
Runs the LongMemEval-S dataset (500 questions, 6 categories) through the
real v5 MCP server (Tantivy BM25 via memory.hybrid_recall).

For each question:
1. Start a fresh `wm serve` process with a fresh tempdir store.
2. Batch-ingest all haystack turns via memory.create (with tags).
3. Search via memory.hybrid_recall (conjunction → relaxed OR fallback).
4. Evaluate Recall@1/5/10, MRR.
5. Kill the process.
...
def evaluate_result(
    ...
    """Check if answer appears in retrieved memories."""
    match_ranks = []
    ...
    for rank, r in enumerate(results, 1):
        ...
        if str(mem_id) in answer_memory_ids:
    ...
    return {
        ...
        "recall_at_10": 1 if match_ranks else 0,
...
def run_benchmark(
    ...
    for qi, item in enumerate(dataset):
        ...
        # Timing: ingest dominates, search is the last request
        ingest_sec = total_sec * 0.95  # approximate split
        latency_ms = (total_sec - ingest_sec) * 1000
        ...
        # ── Phase 3: Evaluate ──────────────────────────────────────────────
        ev = evaluate_result(results, answer, answer_memory_ids)
