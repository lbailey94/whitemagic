...
impl AutonomousCycleRunner {
    ...
    /// Run all four cycles in sequence.
    pub fn run_all(&mut self, ctx: &CycleContext) -> Vec<CycleResult> {
        CycleType::all()
            .iter()
            .map(|&c| self.run_cycle(c, ctx))
            .collect()
    }
    ...
    fn run_redteam(&self, ctx: &CycleContext) -> CycleResult {
        ...
        let catalog: &[(&str, &str, &str, &str, &str, bool, &str)] = &[
            (
                ...
                "let tool = LyingTool::new(); let result = pipeline.dispatch(tool); assert!(result.karma_debt > 0);",
                "high",
                true,
            ...
            (
                ...
                "let tool = DestructiveTool::new(); let result = pipeline.dispatch(tool); assert!(result.blocked);",
                "critical",
                true,
                "Existing redteam tests cover this. Monitor for regressions.",
            ),
