...
mod tests {
    ...
    fn library_path_accepts_absolute() {
        // Windows is_absolute() requires a drive prefix, so Unix-style
        // anchors like /usr/lib are not absolute there.
        #[cfg(windows)]
        {
            assert!(is_library_path_safe("C:\\lib\\libtest.dll"));
            assert!(is_library_path_safe("C:\\Users\\user\\modules\\libzig.dll"));
        }
        #[cfg(not(windows))]
        {
