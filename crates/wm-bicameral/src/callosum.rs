...
impl CorpusCallosum {
    ...
    /// Send a message with optional conformal uncertainty validation.
    ///
    /// Evaluates nonconformity scores against calibrated conformal prediction intervals.
    /// If nonconformity exceeds `max_nonconformity`, the message is rejected.
    pub fn send_conformal(
        &self,
        message: Message,
        nonconformity: Option<f64>,
        max_nonconformity: Option<f64>,
    ) -> bool {
        if let (Some(score), Some(limit)) = (nonconformity, max_nonconformity) {
            if score > limit {
                return false;
            }
        }
