...
impl HarmonyVector {
    ...
    pub fn health_score(&self) -> f32 {
        // Platform honesty: if all physical metrics are exactly zero, sensors are unavailable
        // (e.g. non-Linux / missing /proc). Report neutral 0.5 rather than false perfect 1.0.
        if self.cpu_load == 0.0
            && self.memory_pressure == 0.0
            && self.swap_usage == 0.0
            && self.temperature_c.unwrap_or(0.0) == 0.0
        {
            return 0.5;
        }
