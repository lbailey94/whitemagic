//! wm-autonomic — BitMamba autonomic layer for WhiteMagic v5 (Phase L1).
//!
...
impl SalienceProcessor {
    ...
    pub fn analyze(&mut self, token_ids: &[i32]) -> SalienceSignal {
        ...
        let alpha = 0.1_f32;
        self.salience_baseline = (1.0 - alpha).mul_add(self.salience_baseline, alpha * salience);
...
impl AutonomicLayer {
    ...
    pub fn status(&self) -> serde_json::Value {
        ...
        serde_json::json!({
            ...
            "recent_signals": self.recent_signals.lock().map_or(0, |s| s.len()),
            "telemetry_buffered": self.telemetry_buffer.lock().map_or(0, |b| b.len()),
        })
    }
    ...
    #[must_use]
    pub fn signal_to_drive_events(signal: &SalienceSignal) -> Vec<crate::drive::DriveEventKind> {
        let mut events = Vec::new();

        match signal.signal_type {
            SignalType::Novelty => {
                events.push(crate::drive::DriveEventKind::NovelInput);
            }
            SignalType::Anomaly => {
                // Anomaly → caution up
                events.push(crate::drive::DriveEventKind::LowConfidence);
            }
            SignalType::EmotionalShift => {
                // Emotional shift → social/curiosity
                events.push(crate::drive::DriveEventKind::NovelInput);
                events.push(crate::drive::DriveEventKind::SocialInteraction);
            }
            SignalType::Background => {
...
mod tests {
    ...
    fn salience_processor_baseline_ema() {
        ...
        assert_ne!(processor.salience_baseline(), initial_baseline);
    }
    ...
    fn signal_to_drive_events_novelty() {
        ...
        assert!(events.contains(&crate::drive::DriveEventKind::NovelInput));
    }
    ...
    fn signal_to_drive_events_anomaly() {
        ...
        let events = AutonomicLayer::signal_to_drive_events(&signal);
        assert!(events.contains(&crate::drive::DriveEventKind::LowConfidence));
    }
    ...
    fn daemon_path_accepts_valid_binary() {
        // Create a real executable file instead of relying on platform paths
        // (/bin/true is Linux-only; Windows runners have neither /bin nor
        // reliable POSIX semantics).
        let tmp = tempfile::tempdir().unwrap();
        let bin = tmp.path().join("daemon-bin");
        #[cfg(unix)]
        {
            use std::os::unix::fs::PermissionsExt;
        ...
        #[cfg(not(unix))]
        std::fs::write(&bin, b"echo ok").unwrap();
        assert!(is_daemon_path_safe(bin.to_str().unwrap()));
    }
