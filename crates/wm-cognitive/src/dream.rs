...
impl DreamCycle {
    ...
    fn phase_consolidation(&mut self, ctx: &DreamContext) -> (usize, usize, usize, bool, String) {
        ...
        for (source, target, min_importance) in pathways {
            ...
            for mem in &source_mems {
                // D1/D8 Dream Hygiene: telemetry, friction logs, and raw noise must never transfer
                // to permanent knowledge galaxies.
    ...
    fn tier_sweep(&self, ctx: &DreamContext) -> (usize, usize) {
        ...
        for (galaxy, mems) in &galaxy_mems {
            for mem in mems {
                ...
                let target = if mem.is_telemetry_or_noise() {
                    // D1/D8 Dream Hygiene: telemetry and noise records never promote to Semantic.
                    // They age out of Working to Episodic and decay out to Archival.
                    match mem.metadata.tier {
    ...
    fn phase_oracle(&self, ctx: &DreamContext) -> (usize, usize, usize, bool, String) {
        ...
        for (_galaxy, mems) in &galaxy_mems {
            for mem in mems {
                // D1/D8 Dream Hygiene: telemetry and friction records are never hubs for hypotheses
                if mem.is_telemetry_or_noise() {
                    continue;
                }
