...
impl StrategySynthesizer {
    ...
    fn cluster_by_tags(&self, mems: &[(Galaxy, Memory)]) -> Vec<MemoryCluster> {
        ...
        // Inverted index: tag -> memory indices containing that tag
        ...
        // Track compared pairs to avoid redundant comparisons across shared tags
        let mut compared: HashSet<(usize, usize)> = HashSet::new();

        for indices in tag_index.values() {
            // Cap pairwise checks for any single ubiquitous tag to avoid quadratic explosion
            let cap = indices.len().min(300);
            for i in 0..cap {
                for j in (i + 1)..cap {
                    let u = indices[i];
                    let v = indices[j];
                    ...
                    if compared.insert(pair) {
                        let sim = jaccard_similarity(&tag_sets[u], &tag_sets[v]);
        ...
        clusters.sort_by_key(|cluster| std::cmp::Reverse(cluster.memory_ids.len()));
        clusters
    }
    ...
    fn synthesize_strategy_memory(
        ...
        let compressed: Vec<String> = snippets
            .iter()
            .map(|s| {
                if let Some((byte_idx, _)) = s.char_indices().nth(100) {
                    format!("{}…", &s[..byte_idx])
                } else {
                    s.to_string()
                }
