...
impl ResourceRules {
    ...
    pub fn human_approved(&self) -> bool {
        self.human_approved.read().is_ok_and(|g| *g)
    }
    ...
    pub fn evaluate(
        ...
        let user_initiated = self.user_initiated.read().map_or(true, |g| *g);
        if !user_initiated && cfg.require_human_review {
            let approved = self.human_approved.read().is_ok_and(|g| *g);
            if !approved {
    ...
    pub fn budget_usage(&self) -> BudgetUsage {
        ...
        BudgetUsage {
            writes_last_minute: self
                .write_budget
                .read()
                .map_or(0, |t| t.count_last_minute(now) as u32),
            ...
            network_last_minute: self
                ...
                .read()
                .map_or(0, |t| t.count_last_minute(now) as u32),
            novelty_entries: self.novelty.read().map_or(0, |n| n.len() as u32),
        }
    }
}
