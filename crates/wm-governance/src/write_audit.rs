...
//! the delta observed while a dispatch was in flight is attributed to that
//! dispatch. Concurrent in-process dispatches can interleave, so a
//! write delta is best-effort evidence, not proof — the honest ceiling is
...
impl WriteAuditJournal {
    ...
    ///
    /// The store mutation delta observed since the previous record is
    /// attributed to this dispatch (best-effort; concurrent dispatches can
    /// interleave).
    ...
    pub fn record(
        ...
        if self.flush_threshold == 0
            || self.pending.lock().map(|p| p.len()).unwrap_or(0) >= self.flush_threshold
        {
            self.flush()?;
        }
