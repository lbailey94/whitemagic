...
impl KarmaLedger {
    ...
    pub fn flush(&self) -> Result<()> {
        ...
        // Untracked: the karma chain is governance bookkeeping, not a memory
        // mutation. A threshold-triggered flush must not land `batch.len()`
        // mutation-counter ticks on whichever dispatch happens to be in
        // flight (write-audit false-positive class, 2026-08-28).
    ...
    pub fn total_debt(&self) -> f32 {
        ...
        let now = wm_core::time::now_unix_secs();
    ...
    pub fn anchors(&self) -> Result<Vec<MerkleCheckpoint>> {
        ...
        all.sort_by_key(|x| std::cmp::Reverse(x.timestamp));
        Ok(all)
    }
}
