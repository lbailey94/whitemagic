...
impl AssociationStore {
    ...
    pub fn traverse_cross_galaxy(
        &self,
        env: &Environment,
        store: &crate::store::MemoryStore,
        seed_id: Uuid,
        min_weight: f32,
    ) -> Result<Vec<(Association, Galaxy, crate::memory::Memory)>> {
        let outgoing = self.find_from(env, seed_id)?;
        let incoming = self.find_to(env, seed_id)?;
        let mut results = Vec::new();
        ...
        for edge in outgoing.into_iter().chain(incoming) {
            ...
            let neighbor_id = if edge.source == seed_id {
                edge.target
            } else {
                edge.source
            };
            ...
            if let Some((galaxy, mem)) = store.find_across_galaxies(neighbor_id)? {
                results.push((edge, galaxy, mem));
            }
        }
