...
impl MandalaLevel {
    ...
    ///
    /// On Windows NTFS materializes the LMDB map file at full size on open
    /// (no sparse ftruncate like Unix), so reserving 1–4GB per compartment
    /// allocates real disk immediately. Windows sizes are therefore smaller;
    /// Unix sizes are unchanged. Callers needing larger Windows maps should
    /// open with an explicit size.
    #[must_use]
    pub const fn map_size(self) -> usize {
        #[cfg(windows)]
        {
            match self {
                Self::Research | Self::Sandbox => 64 * 1024 * 1024, // 64 MB
                Self::Production => 128 * 1024 * 1024,              // 128 MB
                Self::Secure => 256 * 1024 * 1024,                  // 256 MB
            }
        }
        #[cfg(not(windows))]
        {
