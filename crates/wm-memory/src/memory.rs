...
pub struct MemoryMetadata {
    ...
    /// Corroborating session ids (bridging consensus counter, 2nd PR after
...
/// Corroboration knob (bridging consensus counter, maintainer ruling 2026-09-04).
///
/// Post-fusion ranking multiplier, off by default: reads
/// `WM_CORROBORATION_WEIGHT` (finite, clamped to [0, 1]; unset/invalid =
/// 0.0). While 0.0 the boost is identically 1.0 and recall is byte-identical
/// with or without corroboration stamps (the S8 doctrine, same as trust).
#[must_use]
...
impl Memory {
    ...
    /// Checks whether this memory is operational telemetry, friction logging, or raw noise
    /// that should be excluded from dream consolidation, tier promotion, and serendipity
    /// association mining (D1/D5/D8 hygiene rules).
    #[must_use]
    pub fn is_telemetry_or_noise(&self) -> bool {
        // Protected memories are never noise
        if self.metadata.is_protected {
            return false;
        }

        // High-signal knowledge tags protect memories from being classified as noise
        ...
        // Tag-based noise detection (D1/D5/D8)
        for tag in &self.metadata.tags {
            let t = tag.to_lowercase();
        ...
        // Content heuristics: raw JSON with telemetry markers
        ...
        // Low importance with minimal content (< 50 chars)
...
mod tests {
    ...
    // ── Bridging consensus counter (2nd PR after Slice B) ─────────────
    ...
    fn corroboration_boost_semantics() {
        // Weight 0 = off: score passes through untouched at any count.
        assert!((corroboration_boost(2.0, 5, 0.0) - 2.0).abs() < 1e-5);
        // Zero corroborations = passthrough at any weight.
        assert!((corroboration_boost(2.0, 0, 1.0) - 2.0).abs() < 1e-5);
        // First corroboration moves most: factor 1 + w/3.
        let first = corroboration_boost(3.0, 1, 0.6);
        assert!((first - 3.6).abs() < 1e-5);
        // Monotone in count, bounded by 1 + weight.
    ...
    fn corroboration_weight_defaults_off() {
        // Load-bearing for the S8 byte-identical doctrine (same caveat as
        // validity: the ON case is covered by inspection, not env mutation).
