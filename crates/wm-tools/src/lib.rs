...
impl Tool for MemoryCreateTool {
    ...
    async fn call(&self, ctx: &mut Context, args: Value) -> wm_core::Result<Value> {
        ...
        // Phase 3 secrets hygiene: credential-shaped content is flagged at
        ...
        // survive export/import roundtrips.
...
impl Tool for MemoryBatchCreateTool {
    ...
    async fn call(&self, ctx: &mut Context, args: Value) -> wm_core::Result<Value> {
        ...
        // Episodic-capture provenance: User only when EVERY item
        // deliberately claimed user (the create-tool rule, derived — the
        // old unconditional User stamp is the 68547b9 miss the build plan
        // flags).
        ...
        for item in items {
            ...
            let mut memory = Memory::new(galaxy, content.to_string());
            memory.metadata.tags = tags;
            // V8 S5: optional importance (the write gate rewrites this to
        ...
        capture_explicit_memories(
            ...
            &memories,
            EpisodicKind::Observation,
            // Derived, not unconditional: agent default, User only when
            // every item deliberately claimed user (mirrors memory.create).
...
impl Tool for MemoryListTool {
    ...
    async fn call(&self, _ctx: &mut Context, args: Value) -> wm_core::Result<Value> {
        ...
        // Scan wide, then filter, then page: offset/limit apply to the
        // VISIBLE surface (private memories and excluded tags never
        // consume page slots).
...
///
/// One governed dispatch for maintenance-scale runs (heritage dedupe,
/// telemetry sweeps): a single round trip, one Tantivy commit for the whole
/// batch, one karma/audit entry. Destructive: requires `confirm: true` and an
/// explicit id list (capped) — the bulk-delete confirm gate from the
/// incident-ledger lessons; there is deliberately no query-form variant.
...
impl Tool for MemoryBatchDeleteTool {
    ...
    async fn call(&self, _ctx: &mut Context, args: Value) -> wm_core::Result<Value> {
        ...
        // Single Tantivy commit for the whole batch (non-fatal on failure).
...
impl Tool for MemoryQueryTool {
    ...
    async fn call(&self, _ctx: &mut Context, args: Value) -> wm_core::Result<Value> {
        ...
        // Time-range passthrough — RFC 3339 bounds map onto the store's
        // temporal filter (previously accepted and silently ignored).
...
mod tests {
    ...
    /// API honesty (§8): `offset` and `exclude_tags` are real. Paging
    /// addresses the VISIBLE surface — private memories and excluded tags
    /// never consume page slots.
    #[tokio::test]
    async fn memory_list_offset_and_exclude_tags_page_visible_surface() {
        ...
        // Baseline: private memory and the excluded tag drop out of the
        // visible surface; the response discloses matched vs returned.
        let all = list
            .call(
                &mut ctx,
        ...
        // Page 1 + page 2 partition the visible surface without overlap.
        let page1 = list
            .call(
                &mut ctx,
    ...
    /// API honesty (§8): `created_after` / `created_before` pass through to
    /// the store's temporal filter instead of being silently ignored.
    #[tokio::test]
    async fn memory_query_time_range_passthrough() {
        ...
        // Malformed bounds are a loud InvalidArgs, never a silent no-filter.
        let bad = query
