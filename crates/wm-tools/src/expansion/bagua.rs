...
impl Trigram {
    ...
    pub const fn doctrine(self) -> &'static str {
        match self {
            Self::Qian => {
                "Sovereign Creation: Synthesis of high-salience realization and alchemical gold."
            }
            Self::Kun => {
                "Receptive Grounding: Dimensional dispersion and topological manifold equilibrium."
            }
            Self::Zhen => {
                "Arousing Shock: Lightning traversal through code graphs and AST symbols."
            }
            Self::Xun => {
                "Pervasive Wind: Deep multi-repository reconnaissance across all workspaces."
            }
            Self::Kan => {
                "Abyssal Ocean: Holographic 5D geometric retrieval from the unconscious sea."
            }
            Self::Li => {
                "Illuminating Flame: Mining and transmuting historical git optimization geneseeds."
            }
            Self::Gen => {
                "Immovable Mountain: Immutability, boundary checks, and Dharma governance."
            }
            Self::Dui => {
                "Joyous Exchange: Sangha consensus, cross-galaxy links, and harmonic resonance."
            }
        }
    }
    ...
    pub fn from_objective(objective: &str) -> Self {
        ...
        {
            Self::Kan
        } else if lower.contains("geneseed")
        ...
        let trigram = if let Some(t_str) = args.get("trigram").and_then(Value::as_str) {
            Trigram::parse_trigram(t_str).ok_or_else(|| {
                CoreError::InvalidArgs(format!(
                    "Unknown trigram: {t_str}. Valid: qian, kun, zhen, xun, kan, li, gen, dui"
                ))
            })?
        } else {
...
mod tests {
    ...
    async fn test_bagua_dispatch_gen_audit() {
        ...
        assert_eq!(
            res["captain_dispatched"],
            "Captain Sentry (Lin - Mountain Dharma)"
        );
    ...
    async fn kun_preserves_dry_run_and_caller_scope() {
        ...
        let codex_mem = Memory::new(
            Galaxy::Codex,
            "Logic algorithm compute binary structure".into(),
        );
        let journals_mem = Memory::new(
            Galaxy::Journals,
            "Dream consolidation nightly summary".into(),
        );
        ...
        let journals_after = store
            .get(Galaxy::Journals, journals_mem.metadata.id)
            .unwrap()
            .unwrap();
        assert_eq!(
