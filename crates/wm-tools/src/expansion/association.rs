...
/// `memory.corroborate` — record a session's corroboration of a memory
/// (bridging consensus counter, 2nd PR after Slice B; maintainer ruling 2026-09-04).
///
/// A corroboration says: this session's agent independently stands behind
/// the memory. Sessions nest agents and machines — sessions are the working
/// unit for now; the record keeps only the session id (agent/machine can be
/// joined from session records later without a schema change).
///
/// Semantics:
/// - corroborator = explicit `session` UUID arg, else the dispatch
///   `Context.session_id`, else `InvalidArgs` (anonymous corroboration is
///   meaningless — independence requires an identity, and it is
///   client-asserted like `user_id`: attribution, never authentication).
/// - idempotent: a repeat corroboration by the same session reports
///   `already_recorded` and changes nothing.
/// - corroborating a non-current (validity-stamped) memory is allowed —
///   history accumulates; the recall surface still filters by validity
///   while enforced.
/// - ranking effect: none unless `WM_CORROBORATION_WEIGHT > 0` (the
///   saturating `corroboration_boost`, disclosed per-result).
pub struct MemoryCorroborateTool {
    store: Arc<MemoryStore>,
    stats: ToolStats,
    effects: EffectRow,
}

impl MemoryCorroborateTool {
    pub fn new(store: Arc<MemoryStore>) -> Self {
        Self {
            store,
            stats: ToolStats::default(),
...
impl Tool for MemoryCorroborateTool {
    ...
    fn description(&self) -> &str {
        "Record this session's corroboration of a memory (bridging consensus counter input; ranking effect only with WM_CORROBORATION_WEIGHT > 0)"
    }
    fn input_schema(&self) -> Value {
        super::common::schema(
            &json!({
                "id": super::common::str_prop("Memory UUID to corroborate"),
                "session": super::common::str_prop("Corroborating session UUID (defaults to the dispatch session; required when the dispatch carries none)"),
            }),
            &["id"],
        )
    }
    async fn call(&self, ctx: &mut Context, args: Value) -> wm_core::Result<Value> {
        let id_str = args.get("id").and_then(Value::as_str).ok_or_else(|| {
            wm_core::CoreError::InvalidArgs("id (memory UUID string) required".into())
        })?;
        let id = uuid::Uuid::parse_str(id_str)
            .map_err(|e| wm_core::CoreError::InvalidArgs(format!("invalid memory UUID: {e}")))?;
        let session = args
            .get("session")
            .and_then(Value::as_str)
            .map(uuid::Uuid::parse_str)
            .transpose()
            .map_err(|e| wm_core::CoreError::InvalidArgs(format!("invalid session UUID: {e}")))?
            .or(ctx.session_id)
            .ok_or_else(|| {
                wm_core::CoreError::InvalidArgs(
                    "session (UUID string) required: the dispatch carries no session_id and none was passed"
                        .into(),
                )
            })?;
        // Galaxy-blind resolution (the graph.walk pattern): corroborate by
        // id wherever the memory lives.
        let mut found = None;
        for galaxy in wm_core::Galaxy::memory_galaxies() {
            if let Some(mem) = self.store.get(galaxy, id)? {
                found = Some((galaxy, mem));
                break;
            }
        }
        let (galaxy, mut mem) = found.ok_or_else(|| {
            wm_core::CoreError::NotFound(format!("memory {id} not found in any memory galaxy"))
        })?;
        if mem.metadata.corroborated_by.contains(&session) {
            return Ok(json!({
                "status": "already_recorded",
                "id": id,
                "galaxy": super::common::galaxy_name(galaxy),
                "corroboration_count": mem.metadata.corroborated_by.len(),
            }));
        }
        mem.metadata.corroborated_by.push(session);
        // No content change: the Tantivy index needs no update.
...
mod tests {
    ...
    /// Bridging counter: recording is idempotent per session, distinct
    /// sessions accumulate, and anonymous corroboration is refused.
    #[tokio::test]
    async fn corroborate_records_distinct_sessions_idempotently() {
        let store = test_store();
        let mem = Memory::new(wm_core::Galaxy::Codex, "a claim".into());
        let id = mem.metadata.id;
        store.put(wm_core::Galaxy::Codex, &mem).unwrap();

        let tool = MemoryCorroborateTool::new(store.clone());
        let mut ctx = Context::default();
        let s1 = uuid::Uuid::new_v4().to_string();
        let s2 = uuid::Uuid::new_v4().to_string();

        let first = tool
            .call(&mut ctx, json!({"id": id.to_string(), "session": s1}))
            .await
            .unwrap();
        assert_eq!(first["status"], "recorded");
        assert_eq!(first["corroboration_count"], 1);

        // Same session again: no-op, count unchanged.
        let repeat = tool
            .call(&mut ctx, json!({"id": id.to_string(), "session": s1}))
            .await
            .unwrap();
        assert_eq!(repeat["status"], "already_recorded");
        assert_eq!(repeat["corroboration_count"], 1);

        // Distinct session accumulates.
        let second = tool
            .call(&mut ctx, json!({"id": id.to_string(), "session": s2}))
            .await
            .unwrap();
        assert_eq!(second["status"], "recorded");
        assert_eq!(second["corroboration_count"], 2);

        // Persisted on the record.
        let stored = store.get(wm_core::Galaxy::Codex, id).unwrap().unwrap();
        assert_eq!(stored.metadata.corroborated_by.len(), 2);
    }

    #[tokio::test]
    async fn corroborate_uses_dispatch_session_then_refuses_anonymous() {
        let store = test_store();
        let mem = Memory::new(wm_core::Galaxy::Codex, "a claim".into());
        let id = mem.metadata.id;
        store.put(wm_core::Galaxy::Codex, &mem).unwrap();
        ...
        let tool = MemoryCorroborateTool::new(store.clone());
        // Dispatch session fills the corroborator.
        ...
        let out = tool
            ...
            .await
            .unwrap();
        assert_eq!(out["status"], "recorded");
        assert_eq!(out["session"], session.to_string());

        // No session anywhere: refused, not minted anonymously.
        let mut bare = Context::default();
        let err = tool
            .call(&mut bare, json!({"id": id.to_string()}))
            .await
            .unwrap_err();
        assert!(matches!(err, wm_core::CoreError::InvalidArgs(_)));

        // Unknown memory: not found.
        let err = tool
            .call(
                &mut bare,
                json!({"id": uuid::Uuid::new_v4().to_string(), "session": session.to_string()}),
            )
            .await
            .unwrap_err();
        assert!(matches!(err, wm_core::CoreError::NotFound(_)));
    }
}
