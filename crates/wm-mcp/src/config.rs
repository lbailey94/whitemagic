...
impl WmConfig {
    ...
    ///   `%USERPROFILE%\.whitemagic` when APPDATA is unavailable.
    /// - Unix: `$XDG_DATA_HOME/whitemagic` or `~/.local/share/whitemagic`.
    ///
    /// The bare `.whitemagic` last resort is cwd-relative and only reachable
    /// in exotic environments without HOME/USERPROFILE.
    fn default_store_dir() -> PathBuf {
        #[cfg(windows)]
        {
            if let Ok(appdata) = std::env::var("APPDATA") {
                return PathBuf::from(appdata).join("whitemagic");
            }
            if let Ok(profile) = std::env::var("USERPROFILE") {
                return PathBuf::from(profile).join(".whitemagic");
            }
        }
