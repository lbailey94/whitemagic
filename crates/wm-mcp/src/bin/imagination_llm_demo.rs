...
use std::sync::Arc;
use wm_bicameral::{
    ScenarioEngine, ScenarioEvaluator, world_model_from_env,
    simulation_bridge::{SimulationBridge, SimulationBridgeConfig},
};
use wm_consciousness::PatternBridge;
...
fn main() {
    ...
    println!("\nLLM Configuration:");
    println!("  WM_LLAMA_ENDPOINT: {}", wm_llama.as_deref().unwrap_or("(not set — using stubs)"));
    println!("  WM_LLM_API_KEY: {}", if wm_llm_key.is_some() { "set (cloud LLM for right hemisphere)" } else { "(not set — using stub right)" });
    ...
    println!("  Expected runtime: ~{}-{} minutes", est_calls * 15 / 60, est_calls * 30 / 60);
    ...
    for (i, s) in scenarios.iter().enumerate() {
        ...
        println!("     score={:.2} risk={:.2} novelty={:.2}", s.score, s.risk, s.novelty);
        if !s.rationale.is_empty() {
            println!("     rationale: {}", s.rationale.chars().take(200).collect::<String>());
        }
        if let Some(ref breakdown) = s.breakdown {
            println!("     breakdown: overall={:.2} goal={:.2} risk={:.2} novelty={:.2} confidence={:.2}",
                breakdown.overall, breakdown.goal_progress,
                breakdown.risk_avoidance, breakdown.novelty, breakdown.confidence);
        }
    }

    if let Some(best) = engine.select_balanced(&scenarios, 0.05) {
        println!("\n  ★ Best (balanced): \"{}\"", best.action.chars().take(120).collect::<String>());
    ...
    println!("  confidence: {:.2}", reflection.actual_prediction.confidence);
    println!("  goal_progress: {:.2}", reflection.actual_prediction.goal_progress);

    println!("\nCounterfactual outcome (predicted):");
    println!("  {}", reflection.counterfactual_prediction.description);
    println!("  confidence: {:.2}", reflection.counterfactual_prediction.confidence);
    println!("  goal_progress: {:.2}", reflection.counterfactual_prediction.goal_progress);
    ...
    println!("\nEnriched scenario: \"{}\"", enriched.scenario.action.chars().take(100).collect::<String>());
    println!("  original score: {:.2}", enriched.scenario.score);
    println!("  adjusted confidence: {:.2}", enriched.adjusted_confidence());
    println!(
    ...
    println!(
        ...
        enriched.prior.n_points,
        enriched.prior.method,
        enriched.prior.mae
    );
    ...
    println!("\nPattern enrichment for {} scenarios:", enriched_scenarios.len());
    for (i, es) in enriched_scenarios.iter().enumerate() {
        ...
        println!(
            ...
            es.novelty.is_familiar,
            es.novelty.novelty_score,
            es.novelty.constellation_count
        );
        println!(
            "     surprise: decision={:?} novelty={:.2} deep_analysis={}",
            es.surprise.decision,
            es.surprise.novelty_score,
            es.surprise.needs_deep_analysis
        );
        println!(
    ...
    // imagine.scenario
    let scenario_tool = ImagineScenarioTool::new(store.clone());
    ...
    if let Ok(val) = result {
        ...
        if let Some(action) = val["best_action"].as_str() {
            println!("  best: \"{}\"", action.chars().take(120).collect::<String>());
        }
    }
    ...
    if let Ok(val) = result {
        ...
        if let Some(desc) = val["best_prediction"]["description"].as_str() {
            println!("  prediction: {}", desc.chars().take(200).collect::<String>());
        }
    ...
    if let Ok(val) = result {
        ...
        if let Some(desc) = val["counterfactual_outcome"]["description"].as_str() {
            println!("  counterfactual: {}", desc.chars().take(150).collect::<String>());
        }
        println!("  would_have_been_better: {}", val["would_have_been_better"]);
    ...
    for q in &queries {
        ...
        println!("  \"{q}\"");
        println!("    → {} (confidence: {:.3})", tool, conf);
    }
