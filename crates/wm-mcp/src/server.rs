...
/// Resource-rules config for this server instance.
///
/// Defaults to `ResourceRulesConfig::default()`; `WM_RESOURCE_MAX_WRITES_PER_MIN`
/// raises the write budget for maintenance one-shots (bulk dedupe/retire runs).
/// Health scaling still applies on top, so the effective limit stays
/// health-aware. Unset/invalid values leave defaults untouched.
#[must_use]
pub fn resource_rules_config_from_env() -> wm_governance::ResourceRulesConfig {
    let mut cfg = wm_governance::ResourceRulesConfig::default();
    if let Ok(v) = std::env::var("WM_RESOURCE_MAX_WRITES_PER_MIN") {
        if let Ok(n) = v.parse::<u32>() {
            cfg.max_writes_per_minute = n;
        }
    }
    cfg
}
...
/// Write-budget sampling cadence (Phase 3): the ledger must not churn the
/// SSD it measures — a stat every 5 min plus a tiny JSON persist is the
/// cost of knowing, which is the point.
const WRITE_BUDGET_SAMPLE_INTERVAL_MS: u64 = 300_000;
...
pub struct McpServer {
    ...
    /// Daily write-budget ledger (Phase 3 irony tax) — `Some` only on
    /// writable servers; `/status` and `wm doctor` render it.
    write_budget: Option<Arc<std::sync::Mutex<wm_substrate::write_budget::WriteBudgetLedger>>>,
    /// Epoch-ms of the last write-budget sample (5-min throttle).
    last_write_budget_sample_ms: std::sync::atomic::AtomicU64,
    ...
    /// success-anomaly memories are written to the store. The read-only
    ...
    /// for servers built outside `with_defaults_mode_profile`.
    profile_contract: Option<wm_tools::profiles::ProfileContract>,
    /// Landlock v0 report from the serve startup (`WM_LANDLOCK=1` opt-in,
    /// whole-process ruleset). `None` for servers built outside the serve
    /// path or when the report was not attached.
    landlock: Option<crate::landlock_sandbox::LandlockReport>,
...
/// Parse the request line and headers of a minimal HTTP/1.1 request head.
///
/// Returns `(method, path, content_length)`. Unknown or absent
/// Content-Length yields 0 (bodies are then treated as absent).
fn parse_http_head(head: &str) -> (String, String, usize) {
    ...
    for (i, line) in head.lines().enumerate() {
        if i == 0 {
            ...
            method = parts.next().unwrap_or_default().to_uppercase();
...
/// Write SSE response headers and flush (connection stays open for events).
async fn write_sse_headers<S>(stream: &mut S) -> std::io::Result<()>
where
    S: tokio::io::AsyncWrite + Unpin,
{
    use tokio::io::AsyncWriteExt;
...
impl McpServer {
    ...
    pub fn new(
        ...
        Self {
            ...
            write_budget: None,
            last_write_budget_sample_ms: std::sync::atomic::AtomicU64::new(0),
    ...
    pub fn with_defaults_mode_profile_preservation(
        ...
        let resource_rules =
            std::sync::Arc::new(ResourceRules::new(resource_rules_config_from_env()));
        ...
        let pipeline = Arc::new(
            ...
            // V8 S5 stage 2c: junk filter + dedup gate + class ceilings/
            // floors on the memory-create path. On read-only servers the
            // read-only gate (1c) blocks writes before stage 2c runs.
        ...
        // registered surface must be exactly the declared one. Violations
        // are logged loud and reported through /status + `wm doctor` —
        // availability stays up, but drift is never silent again.
        let contract = wm_tools::profiles::profile_contract(&full_registry, &registry, profile);
        if !contract.ok {
            tracing::error!(
                profile = %contract.profile,
                expected = contract.expected_count,
                registered = contract.registered_count,
                dead_prefixes = ?contract.dead_prefixes,
                unexpected_tools = ?contract.unexpected_tools,
                "profile contract violation — advertised tool surface does not match the declared profile"
            );
        ...
        if !readonly {
            let root = store_path.parent().unwrap_or(store_path).to_path_buf();
            wm_tools::profiles::save_contract(&root, &contract);
        }
        ...
        // Phase 3 irony tax: track daily store growth on writable servers.
        // Read-only servers never own a ledger (they write nothing and must
        // not create state files); `wm doctor` reads the ledger file itself.
        if !readonly {
            let root = store_path.parent().unwrap_or(store_path).to_path_buf();
            server.write_budget = Some(Arc::new(std::sync::Mutex::new(
                wm_substrate::write_budget::WriteBudgetLedger::load(&root),
            )));
        }
        ...
        // Attach LearnedDreamCycle, AutonomousSmarana, and PhagicCognitiveCoordinator for adaptive consolidation
    ...
    /// Run the server over HTTP + SSE (MCP Streamable HTTP transport).
    ///
    /// Routes:
    /// - `GET /healthz` — liveness probe
    /// - `POST /mcp` — JSON-RPC request, response returned inline on the POST
    ///   connection (server-initiated push is out of scope for v1; the GET
    ///   stream is held open with heartbeat comments only)
    /// - `GET /mcp` — SSE stream (held open, heartbeat comments only)
    ///
    /// Connections are handled sequentially: `McpServer::handle` takes
    /// `&mut self`, and requests are short-lived relative to client wait
        ...
        loop {
            tokio::select! {
                ...
                accepted = listener.accept() => {
                    ...
                    // Bound per-connection wall time so a stalled client
                    // cannot pin the (sequential) handler forever.
                    match tokio::time::timeout(
                        std::time::Duration::from_secs(60),
                        self.handle_http_conn(stream),
                    )
                    .await
                    {
                        Ok(Ok(())) => {}
                        Ok(Err(e)) => tracing::debug!(peer = %peer, error = %e, "HTTP connection ended with error"),
                        Err(_) => tracing::debug!(peer = %peer, "HTTP connection timed out"),
                    }
                }
            }
        }
    ...
    /// Handle a single HTTP connection (head + optional body, then route).
    async fn handle_http_conn(&mut self, mut stream: tokio::net::TcpStream) -> anyhow::Result<()> {
        ...
        let head_end = loop {
            ...
            if n == 0 {
                return Ok(()); // client closed before completing the head
            }
        ...
        let (method, path, content_length) = parse_http_head(&head_text);
        let route = path.split('?').next().unwrap_or_default();

        // Read the body, if any (bounded by the JSON-RPC request cap).
        ...
        if content_length > 0 {
            ...
            let buffered = (buf.len() - head_end).min(content_length);
            body[..buffered].copy_from_slice(&buf[head_end..head_end + buffered]);
            let mut got = buffered;
            while got < content_length {
                let n = stream.read(&mut body[got..]).await?;
                if n == 0 {
                    return Ok(());
                }
                got += n;
        ...
        match (method.as_str(), route) {
            ...
            ("GET", "/status") => {
                ...
                // healthy and how much is it costing the SSD?" without LMDB.
            ...
            ("POST", "/mcp") => {
                let Ok(payload) = std::str::from_utf8(&body) else {
                    write_http_response(&mut stream, 400, "text/plain", b"body must be UTF-8\n")
                        .await?;
                    return Ok(());
                };
    ...
    /// Build the `/status` payload — read-only (a status probe never
    /// writes, so the write-budget report samples sizes without persisting).
    fn status_payload(&self) -> serde_json::Value {
        ...
        json!({
            ...
            "harmony": {
                ...
                "thermal_state": format!("{:?}", hv.thermal_state),
                "temperature_c": hv.temperature_c,
                "battery_percent": hv.battery_percent,
                "disk_io_rate": hv.disk_io_rate,
    ...
    pub fn shutdown(&self) {
        ...
        // Final write-budget sample so the ledger reflects the session's
        // total growth, not just the last 5-min tick (writable only).
        if !self.readonly {
            if let Some(ledger) = &self.write_budget {
                if let Ok(mut ledger) = ledger.lock() {
                    ledger.observe(chrono::Utc::now());
                }
            }
        }
    ...
    /// Get an owned handle to the resource rules (the `store_arc()`
    /// pattern, S7): the daemon hands the SAME Yama budget instance to
    /// dream-phase contexts without holding a borrow of the server
    /// (the dream context must outlive the shared reborrow).
    #[must_use]
    pub fn resource_rules_arc(&self) -> std::sync::Arc<ResourceRules> {
        std::sync::Arc::clone(&self.resource_rules)
    }
    ...
    async fn handle(&mut self, req: &RpcRequest) -> RpcResponse {
        // Agent-presence signal: any request proves the agent is active
        // (the mesh re-announces presence transitions to connected peers).
        if let Some(node) = self.mesh_slot.get() {
    ...
    fn handle_initialize(&self) -> Result<Value, RpcError> {
        ...
        // Phase 2 coordination protocol — the rules ship to every connected
    ...
    /// Handle `resources/list` — returns all Grimoire chapters and truth table.
    ...
    /// Handle `resources/read` — returns resource contents for a given uri.
    fn handle_resources_read(&self, params: &Value) -> Result<Value, RpcError> {
        let uri = params
            ...
            .ok_or_else(|| RpcError {
                code: -32602,
                message: "Missing 'uri' in params".into(),
                data: None,
            })?;

        if let Some(text) = crate::resources::read_resource(uri) {
            Ok(json!({
                "contents": [
                    {
                        "uri": uri,
    ...
    async fn handle_tools_call(&mut self, params: &Value) -> Result<Value, RpcError> {
        ...
        let arg_size = serde_json::to_vec(&arguments).map_or(0, |v| v.len());
        ...
        let response_size = match &dispatch_result {
            Ok(v) => serde_json::to_vec(v).map_or(0, |v| v.len()),
            Err(_) => 0,
        };
        ...
        if success {
            if self.friction_autolog {
                ...
                // the store, not a memory) so failures stay observable
                // without writing the store.
                self.append_friction_ro(&telemetry);
            }
        } else if self.readonly {
            // Failure + read-only: sidecar trace (LMDB friction logging is
            // suppressed in read-only mode).
            self.append_friction_ro(&telemetry);
        } else {
            // FAILURE RECORDING IS DOCTRINE, NOT AUTOLOG NOISE — always on
            // for writable stores (P0.1 pin:
            // e2e_wm_inner_failure_recorded_as_failure_telemetry). Merge
            // resolution 2026-08-31: bea8554's blanket WM_FRICTION_AUTOLOG
            // gate had silenced this branch too; the gate now governs only
            // the success-anomaly branches above. With anomalies gated off
            // the auto-logger initiates no writes in default mode, so the
            // self-amplifying refusal loop bea8554 fixed cannot re-enter
            // through this branch.
        ...
        // ── Phase 3: write-budget sampling ──
        // Throttled to 5 min: a stat on data.mdb, an occasional Tantivy
        // walk, one tiny JSON persist. The irony tax must not itself be
        // an irony — this is the cheapest honest measurement.
        if !self.readonly && self.write_budget.is_some() {
            let wb_epoch_ms = sample_epoch_ms;
            let last_wb_ms = self
                .last_write_budget_sample_ms
                .load(std::sync::atomic::Ordering::Relaxed);
            if wb_epoch_ms.saturating_sub(last_wb_ms) >= WRITE_BUDGET_SAMPLE_INTERVAL_MS {
                self.last_write_budget_sample_ms
                    .store(wb_epoch_ms, std::sync::atomic::Ordering::Relaxed);
                if let Some(ledger) = &self.write_budget {
                    if let Ok(mut ledger) = ledger.lock() {
                        let delta = ledger.observe(chrono::Utc::now());
                        if delta.attributed_bytes > 0 {
                            tracing::debug!(
                                bytes = delta.attributed_bytes,
                                "write-budget sample attributed store growth"
                            );
                        }
                    }
                }
...
mod tests {
    ...
    fn http_head_parsing_extracts_method_path_and_length() {
        ...
        let (method, path, length) = parse_http_head(head);
        assert_eq!(method, "POST");
        assert_eq!(path, "/mcp?x=1");
        assert_eq!(length, 42);
        ...
        let (method, _, length) = parse_http_head(head);
        assert_eq!(method, "POST"); // uppercased
        assert_eq!(length, 7); // case-insensitive header name
    }
    ...
    fn find_subslice_locates_header_terminator() {
        ...
        assert_eq!(&raw[pos..], b"\r\n\r\nbody");
        assert!(find_subslice(b"no terminator", b"\r\n\r\n").is_none());
    }

    #[tokio::test]
    async fn status_payload_reports_write_budget_when_present() {
        let mut server = test_server();
        // No ledger attached → /status must degrade to null, not fail.
        let empty = server.status_payload();
        assert_eq!(empty["status"], "ok");
        assert!(empty["write_budget"].is_null(), "got: {empty}");

        // Attach a ledger over a fake store with real growth (Phase 3 wiring
        // shape: writable servers only).
        let tmp = tempfile::tempdir().unwrap();
        let root = tmp.path().to_path_buf();
        std::fs::create_dir_all(root.join("lmdb/tantivy")).unwrap();
        std::fs::write(root.join("lmdb/data.mdb"), vec![0u8; 4096]).unwrap();
        let ledger = Arc::new(std::sync::Mutex::new(
            wm_substrate::write_budget::WriteBudgetLedger::load(&root),
        ));
        {
            let mut l = ledger.lock().unwrap();
            l.observe(chrono::Utc::now());
        }
        std::fs::write(root.join("lmdb/data.mdb"), vec![0u8; 8192]).unwrap();
        {
            let mut l = ledger.lock().unwrap();
            l.observe(chrono::Utc::now());
        }
        server.write_budget = Some(ledger);
        server.readonly = false;

        let payload = server.status_payload();
        assert_eq!(
            payload["write_budget"]["report"]["today_bytes"], 4096,
            "got: {payload}"
        );
        assert_eq!(payload["write_budget"]["report"]["lmdb_bytes"], 8192);
        assert!(payload["harmony"]["health_score"].is_number());
    }

    #[tokio::test]
    async fn status_payload_on_readonly_server_has_null_write_budget() {
        let mut server = test_server();
        server.readonly = true;
        let payload = server.status_payload();
        assert_eq!(payload["readonly"], true);
        assert!(payload["write_budget"].is_null());
    }

    #[tokio::test]
    async fn status_payload_discloses_landlock_report_when_attached() {
        let mut server = test_server();
        assert!(payload_landlock_is_null(&server), "default is off/null");

        let mut report = crate::landlock_sandbox::LandlockReport::off(std::path::Path::new("/x"));
        report.enabled = true;
        report.outcome = crate::landlock_sandbox::LandlockOutcome::Enforced;
        report.detail = "test".into();
        server.set_landlock_report(report);

        let payload = server.status_payload();
        assert_eq!(payload["landlock"]["enabled"], true, "got: {payload}");
        assert_eq!(payload["landlock"]["outcome"], "enforced");
        assert_eq!(payload["landlock"]["store_root"], "/x");
    }

    fn payload_landlock_is_null(server: &McpServer) -> bool {
    ...
    fn test_server() -> McpServer {
        ...
        let resource_rules = Arc::new(ResourceRules::new(resource_rules_config_from_env()));
    ...
    async fn initialize_returns_server_info() {
        ...
        // Phase 2 coordination stanza: the protocol ships on every
        // handshake; tool usage is advertised only when registered.
        for expected in [
            "one writer per checkout",
            "Claim a scope before shared-tree edits",
            "lease_id",
        ] {
            assert!(
                instructions.contains(expected),
                "coordination stanza must mention {expected}"
            );
        }
    }

    #[tokio::test]
    async fn initialize_advertises_code_claim_only_when_registered() {
        let mut server = test_server();
        assert!(
            server.registry.get("code.claim").is_some(),
            "test server builds the full registry, so coordination tools register"
        );
        let req = RpcRequest {
            jsonrpc: "2.0".into(),
            id: Some(json!(1)),
        ...
        let instructions = resp.result.unwrap()["instructions"]
            ...
            .to_string();
        assert!(
            instructions.contains("code.claim")
                && instructions.contains("coordination signals, not locks"),
            "full-surface servers must advertise the coordination tools, got: {instructions}"
        );

        // Gating logic: drop the code.* tools (what profile filtering does
        // on the curated surface) and the usage line must vanish.
        let mut builder = wm_dispatch::ToolRegistryBuilder::new();
        for tool in server.registry.all() {
            if !tool.name().starts_with("code.") {
                builder.register(tool);
            }
        }
        server.registry = builder.build();
        let resp = server.handle(&req).await;
        let result = resp.result.unwrap();
        let instructions = result["instructions"].as_str().unwrap();
        assert!(
            !instructions.contains("code.claim"),
            "usage line must not appear when the tool is absent (curated profile), got: {instructions}"
        );
        // The protocol rules themselves stay — they are zero-code.
        assert!(instructions.contains("one writer per checkout"));
    }
    ...
    async fn tools_list_returns_only_wm_meta_tool() {
        ...
        // The count must be the live active-registry count, not a hardcoded
        // archive size — this assertion survives tool additions. (The
        // curated-profile counterpart lives in
        // tools_list_reflects_active_profile; here the full surface
        // legitimately reports its own count.)
        let expected_count = server
            .registry
            .all()
            .iter()
    ...
    #[tokio::test]
    async fn handle_request_resources_list_and_read() {
        let mut server = test_server();
        // 1. Check initialize advertises resources capability
        let init_resp = server
            .handle_request(r#"{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}"#)
            .await;
        let init_parsed: Value = serde_json::from_str(&init_resp).unwrap();
        assert!(init_parsed["result"]["capabilities"]["resources"].is_object());

        // 2. List resources
        let list_resp = server
            .handle_request(r#"{"jsonrpc":"2.0","id":2,"method":"resources/list","params":{}}"#)
            .await;
        let list_parsed: Value = serde_json::from_str(&list_resp).unwrap();
        assert!(list_parsed["error"].is_null());
        let resources = list_parsed["result"]["resources"].as_array().unwrap();
        assert_eq!(resources.len(), 31);
        assert_eq!(resources[0]["uri"], "grimoire://chapter/01");
        assert_eq!(resources[30]["uri"], "grimoire://truth-table");

        // 3. Read chapter resource
        let read_resp = server
            .handle_request(r#"{"jsonrpc":"2.0","id":3,"method":"resources/read","params":{"uri":"grimoire://chapter/01"}}"#)
            .await;
        let read_parsed: Value = serde_json::from_str(&read_resp).unwrap();
        assert!(read_parsed["error"].is_null());
        let contents = read_parsed["result"]["contents"].as_array().unwrap();
        assert_eq!(contents.len(), 1);
        assert_eq!(contents[0]["uri"], "grimoire://chapter/01");
        assert!(
            contents[0]["text"]
                .as_str()
                .unwrap()
                .contains("Chapter 01: The Horn")
        );

        // 4. Read truth-table resource
        let tt_resp = server
            .handle_request(r#"{"jsonrpc":"2.0","id":4,"method":"resources/read","params":{"uri":"grimoire://truth-table"}}"#)
            .await;
        let tt_parsed: Value = serde_json::from_str(&tt_resp).unwrap();
        assert!(tt_parsed["error"].is_null());
        let tt_contents = tt_parsed["result"]["contents"].as_array().unwrap();
        assert!(
            tt_contents[0]["text"]
                .as_str()
                .unwrap()
                .contains("72-Engine Truth Table")
        );
    }
    ...
    #[tokio::test]
    async fn profile_contract_attaches_and_persists_on_writable_server() {
        let tmp = tempfile::tempdir().unwrap();
        ...
        let contract = server
            .profile_contract
            .as_ref()
            .expect("contract attached at construction");
        assert!(
            contract.ok,
            "curated surface must satisfy its own contract: {contract:?}"
        );
        assert_eq!(contract.profile, "curated");
        assert_eq!(contract.expected_count, contract.registered_count);
        assert!(contract.dead_prefixes.is_empty());
        // memory.delete rides the `memory` prefix by design — confirm-gated.
        assert!(
            contract
                .destructive_tools
                .iter()
                .any(|t| t == "memory.delete")
        );

        let root = store.parent().unwrap();
        let body = std::fs::read_to_string(root.join("profile_contract.json")).unwrap();
        let persisted: wm_tools::profiles::ProfileContract = serde_json::from_str(&body).unwrap();
        assert!(persisted.ok);
        assert_eq!(persisted.registered_count, contract.registered_count);

        let payload = server.status_payload();
        assert_eq!(payload["profile_contract"]["profile"], "curated");
        assert_eq!(payload["profile_contract"]["ok"], true);
    }
    ...
    async fn profile_contract_on_readonly_server_persists_nothing() {
        ...
        .unwrap();
        assert!(server.profile_contract.as_ref().expect("attached").ok);
        assert!(
            !store
                .parent()
                .unwrap()
                .join("profile_contract.json")
                .exists()
        );
    }
    ...
    async fn tools_list_reflects_active_profile() {
        ...
        // ("229 tools") regardless of the active profile. The durable
        // property is that the description names the ACTIVE surface.
        let list_resp = server
        ...
        let description = parsed["result"]["tools"][0]["description"]
            ...
            .unwrap();
        assert!(
            description.contains("curated tool surface"),
            "curated tools/list description must name the active surface, got: {description}"
        );
