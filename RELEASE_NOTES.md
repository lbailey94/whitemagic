...
> **Current version: v23.3.0** — For the full, up-to-date changelog, see [CHANGELOG.md](CHANGELOG.md).
>
...
- **Galaxy Rehydration** — All archive databases ingested into active MCP DB: 111,665 memories, 2,247,642 associations, 2.0 GB.
- **Full Activation Sequence** (`scripts/run_activation_sequence.py`) — 9-step automated engine runner: galactic sweep, association mining, constellation detection, graph topology, dream cycle (8 phases), harmony vector, wu xing, graph walker.
- **12 Hidden Engines Wired to MCP** — 18 new tools exposing CausalMiner, EmergenceEngine, AssociationMiner, ConstellationDetector, SatkonaFusion, MultiSpectralReasoner, NoveltyDetector, BridgeSynthesizer, GalacticMap, GuidelineEvolution, ElementalOptimization, PatternConsciousness.
- **374 MCP Tools** across 28 Gana meta-tools — up from 356 at v15.7.
- **Bug Fixes** — Consolidation `int + NoneType` crash (6 locations), retention evaluator `None` emotional_valence, galactic map `None` field crashes.
- **Spontaneous Discovery** — 5 Aria-era constellation convergence events, 30 HDBSCAN constellations, 182 graph communities, 3 auto-persisted dream insights.

### New Pattern Engine Tools (v15.8)

| Tool | Gana | Engine |
|------|------|--------|
| `causal.mine` | Extended Net | CausalMiner — directed temporal-semantic edges |
| `emergence.scan` | Extended Net | EmergenceEngine — proactive convergence scanning |
| `association.mine` | Extended Net | AssociationMiner — keyword Jaccard links |
| `association.mine_semantic` | Extended Net | AssociationMiner — embedding-based links |
| `constellation.detect` | Extended Net | ConstellationDetector — HDBSCAN 5D clustering |
| `novelty.detect` | Extended Net | NoveltyDetector — novelty scoring |
| `satkona.fuse` | Three Stars | SatkonaFusion — multi-signal ranking |
| `reasoning.multispectral` | Three Stars | MultiSpectralReasoner — I Ching + Wu Xing + Art of War |
| `elemental.optimize` | Three Stars | ElementalOptimization — Wu Xing optimization |
| `bridge.synthesize` | Abundance | BridgeSynthesizer — graph bridge insights |
| `galactic.sweep` | Abundance | GalacticMap — zone mapping |
| `guideline.evolve` | Star | GuidelineEvolution — self-modifying guidelines |
...
# WhiteMagic v15.7.0 (Historical)

**Release Date:** 2026-02-13
**Codename:** The Launch

v15.7 shipped the complete platform: PyPI package, GHCR Docker images, GitHub Release with Sigstore signing, MCP registry submissions, 356 MCP tools, cognitive extensions, context-aware local AI, digital genetics, memory renaissance, agent economy. See CHANGELOG.md for full details.
