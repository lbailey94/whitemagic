...
def _get_hmac_key() -> bytes:
    ...
    The key is persisted to disk to ensure token verification works across
    process restarts. If the key cannot be persisted, an error is raised since
    this would make all tokens unverifiable after restart.
    """
    global _HMAC_KEY
    ...
    with _HMAC_LOCK:
        ...
        try:
            ...
            raise RuntimeError(
                "Failed to initialize engagement token signing key. "
                "The key must be persisted to disk for token verification to work "
                "across process restarts. Check file system permissions and ensure "
                f"WM_ROOT/security directory is writable. Error: {e}"
            ) from e
    return _HMAC_KEY
...
class EngagementTokenManager:
    ...
    def _persist(self) -> None:
        ...
        try:
            ...
            # Snapshot data under lock, then write outside lock to avoid blocking
            with self._lock:
                data = {
                    tid: {
                        "scope": t.scope,
