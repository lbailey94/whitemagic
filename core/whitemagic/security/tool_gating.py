...
class ToolGate:
    ...
    def _is_url_allowed(self, url: str) -> bool:
        ...
        Uses proper URL parsing and IP address checks instead of string prefix matching.
        """
        import ipaddress
        import socket
        ...
        blocked_schemes = ["data", "file", "ftp", "gopher"]
        try:
        ...
        if self.deployment_mode == "local":
            local_hostname = parsed.hostname or ""
            if local_hostname in ("localhost", "127.0.0.1", "::1"):
                return True
    ...
    def sanitize_error(self, error: Exception) -> str:
        """Sanitize error messages for external output.
