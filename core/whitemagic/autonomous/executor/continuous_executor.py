...
class ContinuousExecutor:
            ...
            while True:
                ...
                if self.has_objectives():
                    ...
                    if len(self.objectives) > 10:
                        ...
                        single_completed = await self.execute_next()
    ...
    async def execute_next(self) -> bool:
        ...
        Returns:
            True when the objective completes successfully, otherwise False.
        """
    ...
    async def edit_file(self, path: str, changes: str) -> None:
        """Edit a file with targeted patching support.

        ``changes`` may be either:

        * A **JSON object** with one of:
          - ``{"start_line": N, "end_line": M, "replacement": "..."}`` — replace
            lines N–M (1-indexed, inclusive) with ``replacement``.
          - ``{"find": "...", "replace": "..."}`` — find-and-replace first
            occurrence of ``find`` with ``replace``.
        * A **plain string** — appended to the file (legacy behaviour, kept for
          backward-compatibility).

        All edits are validated to stay within the sandbox root.
        """
        ...
        if patch is not None:
            ...
            if "start_line" in patch and "end_line" in patch and "replacement" in patch:
                # Line-range replacement (1-indexed, inclusive)
                lines = text.splitlines(keepends=True)
                start = int(patch["start_line"]) - 1
                end = int(patch["end_line"])  # exclusive slice
            ...
            with open(path, "a", encoding="utf-8") as f:
                f.write(changes)
            self.log(f"Appended to file: {path}")
