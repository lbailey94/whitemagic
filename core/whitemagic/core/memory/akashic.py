...
class AkashicField:
    ...
    def _load_field(self) -> None:
        ...
        for seed_id, seed_data in seeds_data.items():
            ...
            seed.last_bloomed = datetime.fromisoformat(seed_data["last_bloomed"]) if seed_data.get("last_bloomed") else None
            seed.planted_at = datetime.fromisoformat(seed_data["planted_at"]) if seed_data.get("planted_at") else datetime.now()
