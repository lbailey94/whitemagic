...
class HolographicMemory:
    ...
    def query_by_vector(self, vector: list[float], k: int = 5) -> list[HolographicResult]:
        """Find k nearest memories to a precomputed 4D/5D vector.

        Args:
            vector: A 4D legacy vector [x, y, z, w] or a 5D vector [x, y, z, w, v].
            k: Number of nearest neighbors to return.
        """
        if not self._has_index:
            return []
        ...
        try:
            if len(vector) == 4:
                query_vector = [vector[0], vector[1], vector[2], vector[3], 0.5]
            elif len(vector) == 5:
                query_vector = vector
            else:
                return []
            ...
            if self._index_5d:
                results = self._index_5d.query_nearest(query_vector, k)
            else:
                assert self._index is not None
                results = self._index.query_nearest(query_vector[0], query_vector[1], query_vector[2], query_vector[3], k)
