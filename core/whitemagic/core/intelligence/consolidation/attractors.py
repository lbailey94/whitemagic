...
class AttractorManager:
    ...
    def _get_coords(self, memory_id: str) -> tuple[float, float, float, float] | None:
        """Helper to get stored 4D coordinates from the unified backend."""
        try:
            unified = getattr(self.manager, "unified", None)
            backend = getattr(unified, "backend", None)
            if backend and hasattr(backend, "get_coords"):
                coords = backend.get_coords(memory_id)
