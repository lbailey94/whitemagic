...
class UniversalInterpreter:
    ...
    def synthesize(
        ...
        actionable_items: dict[str, list[str]] | None = None,
    ) -> str:
        ...
        report.append("\n## Executive Summary")
        report.append(
            "Moving beyond abstract architectural goals, this report surfaces "
        )
        report.append(
            "**specific functional directives** extracted from the deep resonance "
        )
        report.append(
            "of your memory data sea. This is no longer a vision; it is a list "
        )
        report.append(
            "of logical implementations already present in the system's latent state."
        )

        report.append("\n## The Tri-Vector Narrative")
        report.append(
            "The following path was selected by evaluating 198k memories through a "
        )
        report.append(
            "three-fold lens: System Latency, Biographical Resonance, and Dynamical "
        )
        report.append(
            "Stability. Implementation of this path creates a closed-loop autonomy."
        )
        report.append(
        ...
        roots = [
            c for c in clusters if not any(edge[1] == str(c["key"]) for edge in edges)
        ]
        for root in roots:
            k = str(root["key"])
            ...
            report.append(f"\n**{p}**")
            report.append(
                f"> This root represents a high-density anchor ({root['density']} memories). "
            )
            report.append(
                "Initializing this core is the primary priority, as it serves as the "
            )
            report.append("computational substrate for all subsequent layers.")

        report.append("\n### Layer 3-4: The Developmental Arc")
        report.append(
            "The following synthesis steps represent the 'Net' phase—where broad patterns "
        )
        ...
        for cluster in clusters:
            ...
            res_str = (
                f"Resonance: {res_val:.2f} (Verified Flow)"
                if res_val > 0
                else "Base Anchor"
            )
            report.append(
        ...
        report.append(
            "1. **Coordinate Alignment**: These insights are spatially anchored; use `holographic_project` to verify real-time resonance."
        )
        report.append(
        ...
        report.append(
            "The next logical step is to turn these long-form insights into **Primitive Actions**. "
        )
        report.append(
