...
class QuantumGraphAdapter:
    ...
    def quantum_enhanced_walk(
        ...
        seed_nodes = [QuantumNode(id=sid, amplitude=1.0 / math.sqrt(len(seed_ids))) for sid in seed_ids]

        if self._config.use_superposition and get_neighbors_func:
            walked = self._quantum.walk_superposition(
                seed_nodes, get_neighbors_func, hops=self._config.superposition_hops,
            )
        else:
            walked = seed_nodes

        if self._config.use_grover and oracle_func and walked:
            oracle_nodes = [QuantumNode(id=n.id, amplitude=n.amplitude, phase=n.phase) for n in walked]
