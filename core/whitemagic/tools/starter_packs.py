...
_PACKS: dict[str, dict[str, Any]] = {
    "quickstart": {
        ...
        "tools": [
            {
                "name": "gnosis",
                "when": "First call — get system health (use compact=true to save tokens)",
            },
            ...
            {"name": "session_bootstrap", "when": "Initialize a working session"},
            {
                ...
                "when": "Check what tools you're allowed to use",
            },
    ...
    "memory": {
        ...
        "tools": [
            {
                "name": "create_memory",
                "when": "Store a new memory with tags and importance",
            },
            ...
            {"name": "batch_read_memories", "when": "Read multiple memories at once"},
            {
                ...
                "when": "Cluster and synthesize related memories",
            },
            {
                "name": "memory.retention_sweep",
                "when": "Evaluate what to keep/forget (dry_run=true first)",
            },
    ...
    "coordination": {
        ...
        "tools": [
            {
                "name": "agent.register",
                "when": "Register this agent with identity and capabilities",
            },
            {
                "name": "agent.list",
                "when": "Discover other agents and their capabilities",
            },
            ...
            {"name": "vote.cast", "when": "Cast a vote in an active session"},
            {
                ...
                "when": "Chain multiple tools together with variable passing",
            },
            {
                "name": "broker.publish",
                "when": "Send messages to other agents via Redis",
            },
        ],
    },
    "introspection": {
        ...
        "tools": [
            {
                "name": "gnosis",
                "when": "Full system snapshot (or compact=true for alerts only)",
            },
            ...
            {"name": "homeostasis.status", "when": "Self-regulation loop status"},
            {
                ...
                "when": "What the system considers most important right now",
            },
            {
                "name": "tool.graph",
                "when": "Tool dependency relationships for planning",
            },
        ...
        "tools": [
            {
                ...
                "when": "Dual-hemisphere analysis (precise vs creative)",
            },
            {
                "name": "thought_clone",
                "when": "Spawn parallel thought clones for a problem",
            },
            {
                "name": "pattern_search",
                "when": "Find patterns across memory and events",
            },
            {"name": "serendipity_surface", "when": "Surface unexpected connections"},
            {
                "name": "solve_optimization",
                "when": "Multi-strategy optimization solver",
            },
        ...
        "tools": [
            {
                ...
                "when": "Dharma evaluation of a proposed action",
            },
            {
                "name": "check_boundaries",
                "when": "Check if an action crosses safety boundaries",
            },
            {"name": "dharma_rules", "when": "List active rules and profile"},
            {
                "name": "set_dharma_profile",
                "when": "Switch between default/creative/secure profiles",
            },
            {
                "name": "karmic_trace",
                "when": "Immutable audit trail of all Dharma evaluations",
            },
            {
                "name": "governor_check_drift",
                "when": "Check if actions are drifting from goals",
            },
            {
                "name": "maturity.assess",
                "when": "Check developmental stage and tool locks",
            },
        ],
    },
    "free_tier": {
        ...
        "tools": [
            ...
            {
                "name": "create_memory",
                "when": "Store preferences, recurring topics, important people",
            },
            {"name": "search_memories", "when": "Find something you stored before"},
            {
                "name": "galaxy.create",
                "when": "Create a personal memory space (e.g. 'Life', 'Work')",
            },
            {"name": "galaxy.switch", "when": "Switch between your memory spaces"},
            {
                "name": "memory.lifecycle_sweep",
                "when": "Let the system tidy old memories automatically",
            },
        ],
    },
    "pro_tier": {
        ...
        "tools": [
            {
                "name": "galaxy.create",
                "when": "Create one galaxy per client for isolated context",
            },
            {
                "name": "galaxy.switch",
                "when": "Switch to a client's galaxy before their session",
            },
            {
                "name": "galaxy.ingest",
                "when": "Bulk-import client documents into their galaxy",
            },
            {
                "name": "hybrid_recall",
                "when": "Cross-session contextual search across client history",
            },
            {
                "name": "pipeline.create",
                "when": "Automate post-session workflows (summarize, update goals)",
            },
            {
                "name": "archaeology_search",
                "when": "Deep-dive into client history across all sessions",
            },
            {
                "name": "gratitude.stats",
                "when": "Track which clients have tipped (for prioritization)",
            },
        ],
    },
    "dev_tier": {
        ...
        "tools": [
            {
                "name": "capabilities",
                "when": "Discover all available tools and feature flags",
            },
            ...
            },
            {
                ...
                "when": "Discover all workflow starter packs",
            },
            {
                "name": "pipeline.create",
                "when": "Chain tools together with variable passing",
            },
            {
                "name": "explain_this",
                "when": "Pre-execution impact preview of any tool",
            },
        ],
    },
    "team_tier": {
        ...
        "tools": [
            {
                "name": "agent.register",
                "when": "Register agents with roles and capabilities",
            },
            {"name": "agent.list", "when": "Discover other agents on the team"},
            {
                "name": "task.distribute",
                "when": "Assign tasks to agents based on capabilities",
            },
            {"name": "vote.create", "when": "Start a team vote for group decisions"},
            {
                "name": "broker.publish",
                "when": "Send messages to other agents via Redis pub/sub",
            },
            {
                "name": "session.handoff",
                "when": "Transfer context between agents or to humans",
            },
            {
                "name": "swarm.decompose",
                "when": "Break complex tasks into subtasks for the team",
            },
        ],
    },
    "enterprise_tier": {
        ...
        "tools": [
            ...
            {
                "name": "karma_report",
                "when": "Full side-effect audit (declared vs actual)",
            },
            {"name": "karmic_trace", "when": "Immutable audit trail for compliance"},
            {
                "name": "governor_validate",
                "when": "Strategic oversight and goal alignment",
            },
            {"name": "security.monitor_status", "when": "Real-time anomaly detection"},
            {
                "name": "mcp_integrity.verify",
                "when": "SHA-256 schema fingerprint verification",
            },
            {
                "name": "audit.export",
                "when": "Export audit logs in MCP-compatible format",
            },
        ],
    },
}
