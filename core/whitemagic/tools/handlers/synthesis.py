...
def handle_kaizen_apply_fixes(**kwargs: Any) -> dict[str, Any]:
    ...
    not_applicable = [
        p.id for p in selected if (not p.auto_fixable) or (not p.fix_action)
    ]
    ...
    if dry_run:
        return {
            "status": "success",
            "dry_run": True,
            "requested": len(fix_ids),
            "missing": missing,
            "not_applicable": not_applicable,
            "would_apply": [p.id for p in applicable],
        }
...
def handle_serendipity_surface(**kwargs: Any) -> dict[str, Any]:
    ...
    return {
        "status": "success",
        "results": [r.__dict__ for r in results] if results else [],
    }
