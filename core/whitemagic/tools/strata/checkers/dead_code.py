...
def check_dead_code(project_path: Path, file_index: FileIndex, findings: List[Finding]):
    ...
    all_methods: Dict[str, List[Tuple[str, Path, int]]] = {}  # class_name -> [(method_name, file, line)]
    all_calls: set = set()
                        ...
                        if isinstance(child, (ast.FunctionDef, ast.AsyncFunctionDef)) and not child.name.startswith("_"):
                            ...
                            for decorator in child.decorator_list:
                                if isinstance(decorator, ast.Name):
                                    if decorator.id in {"field_validator", "model_validator", "classmethod", "staticmethod", "property"}:
                        ...
                        for decorator in node.decorator_list:
                            if isinstance(decorator, ast.Call):
                                if isinstance(decorator.func, ast.Attribute):
                                    ...
                                    if dec_name in {"get", "post", "put", "delete", "patch", "route", "endpoint", "exception_handler", "on_event", "before_request", "after_request"}:
                                        has_framework_decorator = True
                                elif isinstance(decorator.func, ast.Name):
                                    if decorator.func.id in {"route", "app", "bp", "contextmanager", "asynccontextmanager"}:
                                        has_framework_decorator = True
                            elif isinstance(decorator, ast.Attribute):
                                if decorator.attr in {"get", "post", "put", "delete", "patch", "route"}:
                                    has_framework_decorator = True
                            elif isinstance(decorator, ast.Name):
                                if decorator.id in {"contextmanager", "asynccontextmanager", "staticmethod", "classmethod", "property", "field_validator", "model_validator"}:
                                    has_framework_decorator = True
                        if has_framework_decorator:
                            continue
    ...
    _DISPATCH_DIRS = ("handlers/", "bridge/", "orchestration/", "fusions", "acceleration/", "middleware", "registry", "edge/")
    ...
    for func_name, (file_path, line_num) in all_functions.items():
        ...
        if any(d in file_str for d in _DISPATCH_DIRS):
            continue
        if func_name not in all_calls and func_name not in {"main", "run", "app", "index", "handler", "setup", "teardown", "create_app", "get_app", "init", "initialize", "configure", "register", "activate", "deactivate"}:
            # Skip common callback/hook/listener naming patterns (dynamically dispatched)
            if any(func_name.endswith(suffix) for suffix in ("_hook", "_callback", "_listener", "_handler", "_observer", "_filter")):
                continue
    ...
    for class_name, methods in all_methods.items():
        for method_name, file_path, line_num in methods:
            ...
            _COMMON_DYNAMIC_METHODS = {
                # Standard public API
                "to_dict", "from_dict", "save", "load", "add", "delete",
                "get", "search", "list", "count", "store", "clear_cache",
                # Garden lifecycle (called via garden registry dispatch)
                "feel_", "on_", "experience_", "enter_", "begin_", "start_",
                "deepen_", "gather_", "encourage", "practice_", "consult_",
                "decompose_", "route_", "complete_", "register_", "cast_",
                "resolve_", "resume_", "checkpoint_", "take_", "create_",
                "configure_", "get_", "set_", "update_", "record_", "track_",
                "start_", "end_", "mark_", "army_", "save_",
                # Agent/swarm methods (called via war_room/swarm dispatch)
                "reconnaissance", "monitor_", "verify_", "status_report",
                "plan_", "merge_", "extract_", "scout_", "run_", "skip",
                "progress_", "recommend_", "get_pool", "run_io", "run_cpu",
                "run_api", "run_db", "read_", "execute_", "is_allowed",
                "assess_", "request_", "resolve_", "sanitize_", "describe",
                "should_route", "register_tool", "pack_", "observe_",
                "get_self_report", "clear_history", "suggest_", "get_sop",
                # Parallel/threading (called via thread pool dispatch)
                "add_task", "get_task", "get_results", "cancel_task",
                "add_stage", "read_batch", "run_fast", "time_callable",
                "get_all_rules", "get_metadata", "get_model_info",
                "add_candidate", "should_forget",
            }
            if method_name in _COMMON_DYNAMIC_METHODS or any(
                method_name.startswith(prefix) for prefix in ("feel_", "on_", "get_", "set_", "start_", "end_", "mark_", "record_", "track_", "update_", "configure_")
            ):
                continue
            ...
            _DISPATCH_CLASS_PATTERNS = (
                "Garden", "Agent", "Core", "Handler", "Provider", "Manager",
                "Engine", "Tracker", "Monitor", "Supervisor", "Executor",
                "Pipeline", "Scheduler", "Bandit", "Portal",
                "Lieutenant", "Campaign", "WarRoom", "Swarm", "Doctrine",
                "Holocron", "Librarian", "Editor", "Planner", "Sandbox",
                "Signer", "Ledger", "Validator", "Gate", "Crab",
                "Benchmark", "IChing", "Consolidation", "Decay",
                "ThreadingManager", "FileReader", "TestRunner",
                "Controller", "Optimizer", "ConversationReader",
                # WhiteMagic-specific dispatch classes
                "Bridge", "Miner", "Orchestrator", "Session", "Backend",
                "Studio", "Council", "Modes", "Intake", "State",
                "Enforcer", "Cache", "Spatial", "Dream", "System",
                "Creative", "Mansion", "Adaptive", "Holographic",
                "Hybrid", "Contract", "Browser", "Koka", "Zodiac",
                "Cognitive", "Recursive", "Fusion", "Embedding",
                # Dispatch table entry points (called via string-based lookup)
                "Handler", "Wrapper", "Router", "Dispatcher",
                "Startup", "Shutdown", "Lifecycle", "Bootstrap",
                "Loader", "Registry", "Factory", "Builder",
                "Adapter", "Proxy", "Facade", "Service",
            )
