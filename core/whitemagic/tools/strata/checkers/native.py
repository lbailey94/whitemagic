...
def check_rust(project_path: Path, file_index: FileIndex, findings: list[Finding]):
    ...
    for rs_file in file_index.files_by_extension(".rs"):
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(
                r"\b\d+\.\d+(?:f32|f64)?\s*==\s*\d+\.\d+(?:f32|f64)?\b", stripped
            ) or re.search(r"\b\w+\s*==\s*\d+\.\d+(?:f32|f64)?\b", stripped):
                findings.append(
                    Finding(
                        severity=FindingSeverity.INFO,
    ...
    for go_file in file_index.files_by_extension(".go"):
        ...
        for i, line in enumerate(lines, 1):
            ...
            for match in re.finditer(
                r"\b(fmt\.Println|fmt\.Printf|log\.Print|println)\s*\(", stripped
            ):
                findings.append(
                    Finding(
                        severity=FindingSeverity.INFO,
        ...
        for i, line in enumerate(lines, 1):
            ...
            if re.search(sql_keywords, stripped, re.IGNORECASE):
                if re.search(
                    r'["\']\s*\+|\+\s*["\']|fmt\.Sprintf|strconv\.Format', stripped
                ):
                    findings.append(
                        Finding(
                            severity=FindingSeverity.WARNING,
            ...
            if re.search(
                r"\b\d+\s*\*\s*time\.(Second|Millisecond|Minute|Hour)", stripped
            ):
                findings.append(
                    Finding(
                        severity=FindingSeverity.INFO,
...
def check_cpp(project_path: Path, file_index: FileIndex, findings: list[Finding]):
    ...
    for c_file in file_index.files_by_extension(".c", ".cpp", ".cc", ".h", ".hpp"):
        ...
        for i, line in enumerate(lines, 1):
            ...
            # unsafe C functions
            for match in re.finditer(
                r"\b(strcpy|strcat|gets|sprintf|scanf)\s*\(", stripped
            ):
                findings.append(
                    Finding(
                        severity=FindingSeverity.WARNING,
