...
def check_ruby(project_path: Path, file_index: FileIndex, findings: List[Finding]):
    ...
    for rb_file in file_index.files_by_extension(".rb"):
        ...
        for i, line in enumerate(lines, 1):
            ...
            for match in re.finditer(r'\b(eval|instance_eval|class_eval|module_eval)\s*\(', stripped):
                findings.append(Finding(
            ...
            # Debug prints
            for match in re.finditer(r'\b(puts|print|p|pp|debugger|binding\.pry|binding\.irb)\b', stripped):
                findings.append(Finding(
                    severity=FindingSeverity.INFO,
