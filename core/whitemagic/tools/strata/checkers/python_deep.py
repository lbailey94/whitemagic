...
_ASYNC_FUNCS = {
    "sleep", "gather", "create_task", "wait_for", "shield",
    "ensure_future", "to_thread", "as_completed",
}

# Logging methods where f-strings hurt performance
_LOG_METHODS = {
    "debug", "info", "warning", "warn", "error",
    "critical", "fatal", "exception", "log",
}

# Builtins that are commonly shadowed and cause subtle bugs
_SHADOWNABLE_BUILTINS = {
    "id", "type", "list", "dict", "set", "str", "int", "float", "bool",
    "bytes", "map", "filter", "range", "slice", "object", "tuple",
    "input", "open", "dir", "vars", "help", "license", "copyright",
    "max", "min", "sum", "all", "any", "abs", "round", "pow",
    "hex", "oct", "bin", "chr", "ord", "repr", "format", "sorted",
    "reversed", "enumerate", "zip", "iter", "next", "callable",
...
@register
def check_mutable_defaults(project_path: Path, file_index: FileIndex, findings: List[Finding]):
...
@register
def check_shadowed_builtins(project_path: Path, file_index: FileIndex, findings: List[Finding]):
    ...
    for py_file in file_index.python_files():
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                is_model = any(
                    (isinstance(d, ast.Name) and d.id in {"dataclass", "BaseModel", "Model"})
                    or (isinstance(d, ast.Call) and isinstance(d.func, ast.Name)
                        and d.func.id in {"dataclass"})
                    or (isinstance(d, ast.Attribute) and d.attr in {"dataclass", "BaseModel"})
                    for d in node.decorator_list
                )
                # Also check bases for SQLAlchemy/Pydantic patterns
                if not is_model:
                    for base in node.bases:
                        if isinstance(base, ast.Name) and base.id in {"Base", "BaseModel", "Model", "DeclarativeBase", "Enum", "TypedDict", "NamedTuple", "IntEnum", "StrEnum"}:
                            is_model = True
                        elif isinstance(base, ast.Attribute) and base.attr in {"Base", "BaseModel", "Model", "DeclarativeBase", "Enum", "TypedDict", "NamedTuple", "IntEnum", "StrEnum"}:
                            is_model = True
                if is_model:
        ...
        def _check_node(node: ast.AST, current_class: str | None = None):
            if isinstance(node, ast.Assign):
                for target in node.targets:
                    if isinstance(target, ast.Name) and target.id in _SHADOWNABLE_BUILTINS:
                ...
                if isinstance(node.target, ast.Name) and node.target.id in _SHADOWNABLE_BUILTINS:
                    findings.append(Finding(
...
@register
def check_import_graph(project_path: Path, file_index: FileIndex, findings: List[Finding]):
    ...
    entry_points = {"__main__", "main", "app", "manage", "cli", "wsgi", "asgi",
                    "run_mcp_lean", "run_mcp", "run_server", "run_eval"}
    ...
    for mod in all_modules:
        ...
        if (base.startswith("run_") or base.startswith("bench_") or
                base.startswith("generate_") or base.startswith("analyze_") or
                base.startswith("ignite_") or base.startswith("map_") or
                base.startswith("unify_") or base.startswith("deep_") or
                base.startswith("system_") or base.startswith("constellation_") or
                base.startswith("python_")):
            continue
...
@register
def check_async_hygiene(project_path: Path, file_index: FileIndex, findings: List[Finding]):
    ...
    for py_file in file_index.python_files():
        ...
        for node in ast.walk(tree):
            ...
            rel = py_file.relative_to(project_path)
            rel_str = str(rel)
            if "ganas/" in rel_str or "run_mcp_lean" in py_file.name or "mcp_api_bridge" in py_file.name:
                continue
            ...
            _ASYNC_INTERFACE_DIRS = (
                "embeddings/", "parallel/", "autonomous/executor/",
                "search/", "cascade/", "gratitude/",
                "core/memory/", "core/dreaming/", "core/polyglot/",
                "core/patterns/", "edge/", "core/bridge/",
                "core/resonance/", "core/async_layer",
            ...
            for child in ast.walk(node):
                if isinstance(child, ast.Call):
                    ...
                    while ancestor is not None and ancestor is not node:
                        ...
                        for potential in ast.walk(node):
                            ...
                            for _f, v in ast.iter_fields(potential):
                                if v is ancestor or (isinstance(v, list) and ancestor in v):
                    ...
                    if isinstance(child.func, ast.Name):
                        ...
                        if isinstance(child.func.value, ast.Name) and child.func.value.id == "asyncio":
                            is_asyncio_call = True
                    if func_name and is_asyncio_call and (func_name in _ASYNC_FUNCS or func_name.endswith("_async")):
...
@register
def check_dataclass_mutable_defaults(project_path: Path, file_index: FileIndex, findings: List[Finding]):
        ...
        for node in ast.walk(tree):
            ...
            for item in node.body:
                if isinstance(item, (ast.AnnAssign, ast.Assign)):
                    ...
                    elif isinstance(value, ast.Call) and isinstance(value.func, ast.Name) and value.func.id == "field":
                        # Check for field(default=[...])
                        for kw in value.keywords:
                            if kw.arg == "default" and isinstance(kw.value, (ast.List, ast.Dict, ast.Set)):
                                findings.append(Finding(
                                    severity=FindingSeverity.WARNING,
                                    category="dataclass_mutable_default",
                                    file=str(py_file.relative_to(project_path)),
...
@register
def check_logging_fstrings(project_path: Path, file_index: FileIndex, findings: List[Finding]):
...
@register
def check_type_hint_drift(project_path: Path, file_index: FileIndex, findings: List[Finding]):
