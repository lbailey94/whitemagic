...
BROWSER_TOOLS: list[ToolDefinition] = [
    ...
    ToolDefinition(
        ...
        input_schema={
            ...
            "properties": {
                "url": {"type": "string", "description": "URL to fetch"},
                "chunk_size": {"type": "integer", "description": "Target chars per chunk (default 2000)", "default": 2000},
                "overlap": {"type": "integer", "description": "Overlap chars between chunks (default 200)", "default": 200},
                "summarize": {"type": "boolean", "description": "Generate summary (default true)", "default": True},
                "focus": {"type": "string", "description": "Optional focus topic to guide summarization"},
                "ollama_model": {"type": "string", "description": "Ollama model for summarization (default gemma3:4b)", "default": "gemma3:4b"},
                "max_chars": {"type": "integer", "description": "Max chars to fetch (default 50000)", "default": 50000},
                "chunk_index": {"type": "integer", "description": "If specified, return only this chunk's full text (progressive loading)"},
            },
            "required": ["url"],
        },
    ),
    ...
    # --- Category search (Exa gap closure) ---
    ToolDefinition(
        ...
        input_schema={
            ...
            "properties": {
                "query": {"type": "string", "description": "Search query"},
                "category": {"type": "string", "enum": ["general", "people", "company", "academic", "code", "docs", "news"],
                             "description": "Category filter (default 'general')", "default": "general"},
    ...
    # --- Deep fetch (full content, no chunk skimming) ---
    ToolDefinition(
        name="deep_fetch",
        ...
        input_schema={
            ...
            "properties": {
                "url": {"type": "string", "description": "URL to fetch"},
                "max_chars": {"type": "integer", "description": "Maximum total characters (default 200000)", "default": 200000},
    ...
    # --- Research repo (DeepWiki gap closure) ---
    ToolDefinition(
        name="research_repo",
        ...
        input_schema={
            ...
            "properties": {
                ...
                "max_pages": {"type": "integer", "description": "Max pages to fetch (default 5)", "default": 5},
                "max_chars_per_page": {"type": "integer", "description": "Max chars per page (default 50000)", "default": 50000},
                "store_memories": {"type": "boolean", "description": "Store as memories for Q&A (default true)", "default": True},
            },
            "required": ["repo"],
        },
    ),

    # --- Research URL (open net generalization) ---
    ToolDefinition(
        name="research_url",
        ...
        input_schema={
            ...
            "properties": {
                ...
                "max_chars": {"type": "integer", "description": "Max chars to retrieve (default 200000)", "default": 200000},
                "timeout": {"type": "number", "description": "Timeout in seconds (default 30)", "default": 30.0},
                "store_memory": {"type": "boolean", "description": "Store as memory for Q&A (default true)", "default": True},
            },
            "required": ["url"],
        },
    ),

    # --- Batch web search (parallel multi-query) ---
    ToolDefinition(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "num_results_per_query": {"type": "integer", "description": "Results per query (default 5)", "default": 5},
                "timeout": {"type": "number", "description": "Timeout per search (default 10s)", "default": 10.0},
                "category": {"type": "string", "description": "Optional category filter for all queries"},
            },
            "required": ["queries"],
        },
    ),

    # --- Rabbit hole recursive spiral research ---
    ToolDefinition(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "max_parallel_terms": {"type": "integer", "description": "Max terms per level (default 12)", "default": 12},
                "num_search_results": {"type": "integer", "description": "Search results per term (default 5)", "default": 5},
                "fetch_top_results": {"type": "integer", "description": "Top results to fetch per term (default 3)", "default": 3},
                "max_chars_per_fetch": {"type": "integer", "description": "Max chars per fetch (default 50000)", "default": 50000},
                "store_memories": {"type": "boolean", "description": "Store as memories (default true)", "default": True},
                "cache_content": {"type": "boolean", "description": "Cache content for re-reading (default true)", "default": True},
                "initial_temperature": {"type": "number", "description": "Starting curiosity temp: 0.0=focused, 0.5=curious, 1.0=wild (default 0.0)", "default": 0.0},
                "temperature_rise": {"type": "number", "description": "Temp rise per depth level — simulates clicking tangential links (default 0.25)", "default": 0.25},
                "novelty_floor": {"type": "number", "description": "Stop if novelty below this for 2 levels — detects same-y echo chambers (default 0.15)", "default": 0.15},
            },
            "required": ["topic"],
        },
    ),

    # --- Web content cache management ---
    ToolDefinition(
        name="web_cache_list",
    ...
    ToolDefinition(
        ...
        input_schema={
            "type": "object",
            "properties": {
                "older_than_hours": {"type": "integer", "description": "Only remove files older than this many hours"},
            },
        },
    ),

    # --- Alchemical procession meta-loop ---
    ToolDefinition(
        ...
        input_schema={
            "type": "object",
            "properties": {
                "task": {"type": "string", "description": "The task to process through the alchemical cycle"},
                "cycles": {"type": "integer", "description": "Number of yin-yang cycles (default 2)", "default": 2},
            },
            "required": ["task"],
        },
    ),
    ...
    ToolDefinition(
        ...
        input_schema={
            "type": "object",
            "properties": {
                "prompt": {"type": "string", "description": "Natural language code generation prompt"},
                "max_iterations": {"type": "integer", "description": "Max improvement iterations (default 3)", "default": 3},
                "score_threshold": {"type": "number", "description": "Score at which to stop (default 0.8)", "default": 0.8},
                "repo_path": {"type": "string", "description": "Optional repo path for context"},
            },
            "required": ["prompt"],
        },
    ),

    # --- Image analysis (OCR + structural) ---
    ToolDefinition(
        name="image_analyze",
        ...
        input_schema={
            ...
            "properties": {
                ...
                "url": {"type": "string", "description": "URL of image to analyze (alternative to image_path)"},
                "extract_text": {"type": "boolean", "description": "Attempt OCR text extraction (default true)", "default": True},
                "max_text_length": {"type": "integer", "description": "Max OCR text length to return (default 5000)", "default": 5000},
            },
        },
    ),

    # --- Parallel reasoning (sequential-thinking gap closure) ---
    ToolDefinition(
        ...
        input_schema={
            ...
            "properties": {
                ...
                "max_branches": {"type": "integer", "description": "Max concurrent branches (default 4)", "default": 4},
                "max_depth": {"type": "integer", "description": "Max thoughts per branch (default 6)", "default": 6},
                "fork_threshold": {"type": "number", "description": "Confidence threshold for forking (default 0.5)", "default": 0.5},
            },
            "required": ["question"],
        },
    ),
