...
def maintenance_reindex() -> None:
    ...
    if HAS_RICH and console:
        ...
        console.print(
            "[dim]This will re-calculate spatial coordinates for all memories.[/dim]\n"
        )
        ...
        try:
            ...
            if not os.path.exists(script_path):
                console.print(
                    f"[red]❌ Error: Maintenance script not found at {script_path}[/red]"
                )
                return
        ...
        subprocess.run(
            [sys.executable, "scripts/reindex_data_sea.py"], env=env, check=False
        )
