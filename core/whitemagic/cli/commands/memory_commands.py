...
@click.pass_context
def remember(
    ctx, content: str, title: str | None, tags: str | None, memory_type: str
) -> None:
...
def context(tier: int) -> None:
    ...
    if tier == 0:
        ...
        result = opt.pack_full_context(
            "recent activity", token_budget=8000, memory_limit=50
        )
...
def memory_list(limit: int) -> None:
    ...
    try:
        ...
        if HAS_RICH and console:
            table = Table(
                title=f"📝 Recent Memories (last {limit})",
                show_header=True,
                header_style="bold magenta",
            )
            table.add_column("ID", style="cyan", no_wrap=True)
    ...
    for hop in range(depth):
        ...
        for tag in list(current_tags):
            search_result = call_tool(
                "search_memories", query=tag, limit=limit, now_override=now
            )
        ...
        results.append(
            {
                "hop": hop + 1,
                "tag_seeds": list(current_tags),
                "memories": hop_memories[:limit],
            }
        )
    ...
    if json_output:
        ...
        if HAS_RICH and console:
            ...
            for hop_data in results:
                ...
                lines = [
                    f"  • {m.get('content', '')[:60]} ({m.get('id', '?')[:8]})"
                    for m in mems
                ]
                console.print(
                    Panel(
                        "\n".join(lines),
                        title=f"Hop {hop_num} — {len(mems)} memories",
                        border_style="blue",
                    )
                )
        else:
