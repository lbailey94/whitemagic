...
def send(
    message: str, channel: str, tags: tuple[str, ...], sender: str, priority: str
) -> None:
    """Send a message"""
...
def read(channel: str, limit: int, priority: str | None) -> None:
    """Read messages"""
    chat_system = get_chat()
    msgs = chat_system.read_messages(
        channel=channel, limit=limit, priority_filter=priority
    )
    click.echo(f"\n=== #{channel} ({len(msgs)} messages) ===\n")
    for m in msgs:
        tags_str = f" [{' '.join(m.tags)}]" if m.tags else ""
        priority_str = (
            f"[Priority: {m.priority.upper()}]" if m.priority != "normal" else ""
        )
        click.echo(
            f"[{m.timestamp.strftime('%H:%M')}] {m.sender_id}{tags_str} {priority_str}: {m.content}"
        )
    click.echo("")
...
def acquire(resource: str, reason: str, ttl: int, sender: str) -> None:
    ...
    if success:
        ...
        if status:
            click.echo(
                f"Failed to lock {resource}. Currently locked by {status.locked_by}: {status.reason}"
            )
        else:
...
def renew(resource: str, ttl: int, sender: str) -> None:
    ...
    if status and status.locked_by == sender:
        ...
        click.echo(
            f"Cannot renew lock on {resource}. Not owned by {sender} or not locked."
        )
...
@click.option(
    ...
    "-s",
    default="open",
    type=click.Choice(["open", "in_progress", "completed", "blocked"]),
    help="Task status",
)
@click.option(
    "--priority",
...
def list_tasks(
    status: str | None, channel: str | None, assigned_to: str | None
) -> None:
    """List tasks, optionally filtered"""
    chat_system = get_chat()
    tasks = chat_system.list_tasks(
        status=status, channel=channel, assigned_to=assigned_to
    )
    if not tasks:
        ...
        for t in tasks:
            ...
            click.echo(
                f"- {t.id}: {t.title} ({t.status}, {assigned_str}, Priority: {t.priority})\n"
            )
...
def generate(channel: str, days: int, priority: str | None, save: bool) -> None:
    ...
    chat_system = get_chat()
    if save:
        summary_file = chat_system.save_summary(
            channel=channel, days=days, priority_filter=priority
        )
        click.echo(f"Summary saved to: {summary_file}")
    else:
        summary_content = chat_system.generate_summary(
            channel=channel, days=days, priority_filter=priority
        )
