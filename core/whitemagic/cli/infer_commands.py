...
@click.option(
    "--mode",
    type=click.Choice(["auto", "fast", "explore", "deep"]),
    default="auto",
    help="Inference mode",
)
@click.option(
    "--memory/--no-memory", default=False, help="Ground response in memory (RAG)"
)
...
def query(ctx, query: str, mode: str, memory: bool, json_flag: bool):
    ...
    try:
        ...
        if json_output:
            ...
            click.echo(f"\n{result.answer}\n")
            click.echo(
                f"[{result.tier} tier | {result.latency_ms:.2f}ms | "
    ...
    try:
        ...
        if json_output:
            ...
            if "kv_cache" in stats:
                ...
                click.echo(
                    f"💾 KV cache: {cache.get('hit_rate_percent', 0):.1f}% hit rate"
                )
            if "hardware" in stats:
                hw = stats["hardware"]["current_state"]
                click.echo(
                    f"🖥️  Hardware: CPU {hw['cpu_percent']:.0f}%, Mem {hw['memory_percent']:.0f}%"
                )
    except Exception as e:
        click.echo(f"Error: {e}", err=True)
        raise click.Abort()
