...
class WarRoom:
    ...
    def plan_campaign(
        ...
        logger.info(
            "Campaign %s planned: '%s' (tactic=%s, phase=%s, clones=%s)",
            campaign_id,
            objective,
            tactic_name,
            wu_xing_phase,
    ...
    async def _execute_chain_stratagems(
        ...
        try:
            ...
            logger.debug(
                "Swarm decomposition failed, defaulting to single subtask: %s",
                e,
                exc_info=True,
            )
        ...
        try:
            ...
            if tokio_clones_available():
                result = tokio_deploy_clones(
                    ...
                    num_clones,
                    strategies
                    or ["direct", "analytical", "adversarial", "creative", "synthesis"],
                )
                if result:
                    return {
                        "clones_deployed": result.get("total_clones", 0),
                        "winner_strategy": result.get("winner", {}).get(
                            "strategy", "?"
                        ),
    ...
    def deploy_campaign(
        ...
        try:
            ...
            assignments = swarm.route(
                swarm_plan.id, engagement_token_id=engagement_token_id
            )
            ...
            logger.debug(
                "Swarm deployment failed for campaign %s: %s", campaign.campaign_id, e
            )
            campaign.phases_completed.append(
                {
                    "name": "Swarm Deployment",
    ...
    def detect_optimal_phase(self) -> dict[str, Any]:
        ...
        try:
            ...
            logger.debug(
                "Wu Xing phase lookup failed, using hour fallback: %s", e, exc_info=True
            )
        ...
        try:
            ...
            if snap.energy < 0.4:
                ...
                result["recommendation"] = (
                    "Low energy detected. Favor reflection over action."
                )
        except Exception as e:
...
def get_war_room() -> WarRoom:
    ...
    if _war_room is None:
        _war_room = WarRoom()
        logger.info(
            "War Room initialized — 9 Commanders, 5 Tactics, 36 Stratagems ready"
        )
    return _war_room
