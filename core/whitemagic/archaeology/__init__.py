...
def mark_read(
    ...
    """
    return cast(
        dict[str, Any], get_archaeologist().mark_read(path, context, note, insight)
    )


def mark_written(
    path: str, context: str | None = None, note: str | None = None
) -> dict[str, Any]:
    """
...
def process_wisdom_archives(
    limit_files: int = 1000, memory_type: str = "long_term"
) -> dict[str, Any]:
    ...
    report_file = (
        Path(PROJECT_ROOT) / "core_system" / "data" / "recovered_memories.jsonl"
    )
    if not report_file.exists():
        return {
            "status": "error",
            "message": "No recovered_memories.jsonl found. Run Chariot dig first.",
        }
            ...
            try:
                ...
                unified.store(
                    ...
                    metadata={
                        ...
                        "grimoire_chapters": finding.get("anthropology", {}).get(
                            "chapters", []
                        ),
                    },
...
def create_daily_wisdom_digest() -> str:
    ...
    recent = unified.search(
        query="recovered", limit=5, memory_type=MemoryType.LONG_TERM
    )
    if not recent:
    ...
    output_path = (
        ...
        / "core_system"
        / "data"
        / f"wisdom_digest_{datetime.now().strftime('%Y%m%d')}.md"
    )
...
def mark_read_async(*args, **kwargs):
    return mark_read(*args, **kwargs)
...
def mark_written_async(*args, **kwargs):
    return mark_written(*args, **kwargs)
...
def extract_wisdom(content: str, source: str):
    return get_wisdom_extractor().extract_wisdom(content, source)
