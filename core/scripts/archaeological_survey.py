...
def survey_database(archaeologist: ChariotArchaeologist, db_path: Path):
    ...
    try:
        ...
        if "metadata" in columns: query_cols.append("metadata")
        if "title" in columns: query_cols.append("title")
        if "galactic_distance" in columns: query_cols.append("galactic_distance")
        if "neuro_score" in columns: query_cols.append("neuro_score")
        ...
        for row in rows:
            ...
            if not metadata_str: metadata_str = "{}"
            ...
            matches = []
            if "wxyz" in combined_text: matches.append("Holographic_WXYZ")
            if "synchronicity" in combined_text: matches.append("Synch_Pattern")
            if "dharma" in combined_text: matches.append("Legacy_Dharma")
            if "protocol" in combined_text: matches.append("Protocol_Ref")
            ...
            score = 0
            if chapters: score = chapters[0]["score"]
...
def generate_expanded_recovery_report(report_file: Path, output_md: Path):
    ...
    for f in findings[:50]: # Top 50
        ...
        report.append(f"| {source} | {name[:40]} | {score} | {matches} | {gana} | {top_chap} |")
...
if __name__ == "__main__":
    ...
    generate_expanded_recovery_report(agent.report_file, output_dir / "HOLOGRAPHIC_REPORT.md")
