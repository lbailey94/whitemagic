...
try:
    ...
    logger.debug(f"❌ Failed to import whitemagic_rs: {e}")
    sys.exit(1)
...
def test_similarity():
    ...
    try:
        ...
        logger.debug(f"Similarity ('{s1}', '{s2}'): {score:.4f}")
        assert score > 0.5, "Similarity seems too low"
    except Exception as e:
        logger.debug(f"❌ Error in fast_similarity: {e}")
...
def test_iching():
    ...
    try:
        ...
        logger.debug(f"Hexagram: {hex_num}")
        logger.debug(f"Lines: {lines}")
    except Exception as e:
        logger.debug(f"❌ Error in iching_cast: {e}")
...
def test_patterns():
    ...
    try:
        ...
        logger.debug(f"❌ Error in extract_patterns: {e}")
