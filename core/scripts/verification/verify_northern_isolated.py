...
async def main():
    ...
    with (
        patch("whitemagic.core.memory.manager.MemoryManager") as MockMemMan,
        patch(
            "whitemagic.intelligence.synthesis.serendipity_engine.get_serendipity_engine"
        ) as MockSerendipity,
        ...
        # Setup Mocks
        MockMemMan.return_value.search_memories.return_value = [
            {"title": "Mock Memory", "relevance": 0.9}
        ]
        ...
        await test_gana(
            "Encampment", EncampmentGana, "session_handoff", session_id="mock_session"
        )
