...
async def main():
    logger.debug("=== Verifying Southern Quadrant Ganas (Isolated) ===")

    # Mock global dependencies
    # Note: We must patch where the function is DEFINED or IMPORTED
    with (
        patch("whitemagic.metrics.get_tracker") as MockTracker,
        patch(
            "whitemagic.intelligence.synthesis.kaizen_engine.get_kaizen_engine"
        ) as MockKaizen,
        patch(
            "whitemagic.cascade.context_synthesizer.get_context_synthesizer"
        ) as MockSynth,
        ...
        patch(
            "whitemagic.cascade.advanced_parallel.AdaptiveParallelExecutor"
        ) as MockPar,
        ...
        class MockContext:
            ...
            def get_dominant_influence(self):
                return self.dominant_influence

            def get_recommended_morphology(self):
                return self.recommended_morphology
        ...
        await test_gana(
            "ExtendedNet", ExtendedNetGana, "search_all_patterns", query="test"
        )
        ...
        # 7. Abundance (Surplus)
        MockEcon.return_value.get_budget_status.return_value = {
