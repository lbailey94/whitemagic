...
class ExceptionBlockFixer(ast.NodeTransformer):
            ...
            if context:
                ...
                if isinstance(new_type, str):
                    ...
                    node = ast.ExceptHandler(
                        type=new_node, name=node.name, body=node.body
                    )
    ...
    def _get_exception_context(self, node: ast.ExceptHandler) -> str | None:
        ...
        # File operations
        if any(
            x in try_source_lower
            for x in ["open(", "read(", "write(", ".read()", ".write()"]
        ):
            return "OSError, FileNotFoundError, PermissionError"

        # JSON operations
        if "json" in try_source_lower and any(
            x in try_source_lower for x in ["loads", "dumps", "load", "dump"]
        ):
            return "json.JSONDecodeError, TypeError"

        # SQLite operations
        if "sqlite" in try_source_lower or any(
            x in try_source_lower
            for x in ["execute(", "cursor(", "commit(", "rollback("]
        ):
        ...
        if any(
            x in try_source_lower
            for x in ["requests.", "urlopen", "http", "connect(", "socket"]
        ):
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Only scan, don't apply fixes"
    )
