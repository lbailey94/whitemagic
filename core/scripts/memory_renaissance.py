...
def phase3_associations(db_path: Path, dry_run: bool = False) -> dict:
                        ...
                        try:
                            conn.execute(
                                """INSERT OR IGNORE INTO associations
                                   ...
                                   VALUES (?, ?, ?, ?)""",
                                (mid, target_entity, rel.predicate, rel.confidence),
                            )
                            relation_count += 1
                        except Exception:
                            pass
    ...
    typed_assoc = conn.execute(
        "SELECT COUNT(*) FROM associations WHERE association_type != 'associated_with'"
    ).fetchone()[0]
...
def main():
    ...
    db_path = get_db_path()
    log.info(f"🧠 Memory Renaissance v15.3")
