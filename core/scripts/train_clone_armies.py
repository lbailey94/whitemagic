...
async def scenario_1_thought_clones():
    ...
    print("Objective: Analyze 'How to optimize SQLite query performance for a memory system'")
    ...
    return {"scenario": "thought_clones", "confidence": result.confidence, "llm_used": result.metadata.get("llm_used", False), "elapsed_s": elapsed}
...
async def scenario_2_immortal_clone():
    ...
    from whitemagic.agents.immortal_clone_v2 import ImmortalClone, Task, CampaignVictoryTracker, ActionType
...
""")
    ...
    tracker = CampaignVictoryTracker(
        victory_conditions=[
            {"id": "error_handling_added", "description": "Error handling added to code"},
    ...
    return {"scenario": "immortal_clone", "actions_taken": actions_count, "vcs_met": len(met), "elapsed_s": elapsed}
...
async def scenario_3_bicameral():
    ...
    print("Objective: Reason about 'Should WhiteMagic use vector DB or SQLite FTS5 for search?'")
    print("Deployment: 2 left (analytical) + 2 right (creative) clones, 2 debate rounds, LLM synthesis\n")

    from whitemagic.core.intelligence.bicameral import get_bicameral_reasoner

    reasoner = get_bicameral_reasoner(left_clones=2, right_clones=2, max_debate_rounds=2)
...
async def scenario_4_code_writing():
    ...
    from whitemagic.optimization.rust_code_writing import deploy_writing_army, code_writing_available
    ...
    module_templates = [
        ("memory/scanner.py", '"""Memory scanner module {i}."""\n\ndef scan():\n    pass\n'),
        ("memory/analyzer.py", '"""Memory analyzer module {i}."""\n\ndef analyze():\n    pass\n'),
        ("memory/optimizer.py", '"""Memory optimizer module {i}."""\n\ndef optimize():\n    pass\n'),
    ]
    ...
    start = time.perf_counter()
    results = deploy_writing_army(tmpdir, operations, clone_count=4, army_id="training-army")
    elapsed = time.perf_counter() - start
...
async def scenario_5_bandit_learning():
    ...
    start = time.perf_counter()
    await army.parallel_explore("How to improve code review processes?", 2, use_tokio=False)
    ...
    start = time.perf_counter()
    await army.parallel_explore("How to optimize API response times?", 2, use_tokio=False)
    ...
    for name, stats in sorted(clone_stats.items(), key=lambda x: x[1]["expected_success_rate"], reverse=True)[:5]:
        print(f"  {name}: α={stats['alpha']:.1f} β={stats['beta']:.1f} "
              f"success_rate={stats['expected_success_rate']:.3f} calls={stats['total_calls']}")

    # Get recommendations
    recommendations = bandit.recommend_clone_strategies("analyze and deploy code", clone_type="thought", k=5)
    print(f"\nTop 5 recommended strategies for next deployment:")
    for rec in recommendations:
        print(f"  {rec['strategy']}: sample={rec['sample']:.3f} ev={rec['expected_value']:.3f} calls={rec['total_calls']}")
...
async def scenario_6_war_room():
    ...
    print("Objective: Execute a campaign to 'audit and fix error handling in a codebase'")
    ...
    campaign = {
        ...
        "victory_conditions": [
            {"id": "error_handling_added", "description": "Error handling added", "target": test_file},
        ],
    }
