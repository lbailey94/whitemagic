...
def run_sqlite_benchmark():
    ...
    for m in memories:
        backend.store_memory(
            m["id"], m["content"], m["memory_type"], m["created_at"],
            m["updated_at"], m["accessed_at"], m["access_count"],
            m["emotional_valence"], m["importance"], m["neuro_score"],
            m["novelty_score"], m["recall_count"], m["half_life_days"],
            m["is_protected"], m["metadata"], m["title"],
            m["galactic_distance"], m["retention_score"], m["last_retention_sweep"],
            m["content_hash"], m["event_time"], m["ingestion_time"],
            m["is_private"], m["model_exclude"]
        )
...
def check_wasm():
    ...
    wasm_path = REPO_ROOT / "whitemagic-math/target/wasm32-unknown-unknown/release/whitemagic_math.wasm"
    if wasm_path.exists():
...
if __name__ == "__main__":
    ...
    print(f"WASM Footprint: {wasm_results['size_kb']:.1f} KB ({wasm_results['status']})")
    print("="*60)
