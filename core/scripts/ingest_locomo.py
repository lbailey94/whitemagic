...
def ingest_longmemeval(data_file, conn):
    ...
    for q_entry in data:
        ...
        for idx, turns in enumerate(haystack_sessions):
            session_id = haystack_session_ids[idx] if idx < len(haystack_session_ids) else str(uuid.uuid4())
            ...
            for t_idx, turn in enumerate(turns):
                ...
                conn.execute("INSERT INTO memories_fts (id, title, content, tags_text) VALUES (?, ?, ?, ?)",
