#!/usr/bin/env python3
"""
WhiteMagic LOC Counter - Safe analysis of codebase composition
"""
...
def generate_report(stats: dict) -> str:
    ...
    lines.append("╠══════════════════════════════════════════════════════════════════╣")
    lines.append(
        f"║  Total: {total_files:,} files | {total_loc:,} lines of code{'':>17}║"
    )
    lines.append("╚══════════════════════════════════════════════════════════════════╝")
    lines.append("")
    ...
    for lang, data in sorted_stats:
        ...
        bar = "█" * bar_len
        lines.append(
            f"{lang:12} │{bar:<{bar_width}}│ {pct:5.1f}% ({data['loc']:,} LOC, {data['files']} files)"
        )
    ...
    lines.append(
        f"  Python:     {python_loc:,} LOC ({python_loc / total_loc * 100:5.1f}%) | {python_files:,} files"
    )
    lines.append(
        f"  Polyglot:   {polyglot_loc:,} LOC ({polyglot_loc / total_loc * 100:5.1f}%) | {polyglot_files:,} files"
    )
    lines.append("")
    ...
    for lang, data in sorted_stats:
        lines.append(
            f"  {lang:12}: {data['loc']:>8,} LOC  │  {data['files']:>4} files  │  {data['loc'] / total_loc * 100:5.1f}%"
        )
...
if __name__ == "__main__":
    ...
    root = Path(
        os.environ.get("WHITEMAGIC_DEV_ROOT")
        or (str(Path.home() / "Desktop" / "whitemagic"))
    ).expanduser()
    ...
    with open(output_path, "w") as f:
        f.write("# WhiteMagic LOC Analysis\n\n")
        f.write(f"```\n{report}\n```\n")
