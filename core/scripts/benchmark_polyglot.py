...
def bench_keyword_extraction():
    ...
    for size_label, text_len in [
        ("small (500 chars)", 500),
        ("medium (3K chars)", 3000),
...
def bench_cosine_similarity():
    ...
    for dim in [64, 128, 384, 768]:
        ...
        py_mean, py_med, py_min, py_p99, _ = timed(
            lambda: _py_cosine(a, b),
            iterations=ITERATIONS,
        )

        if status["has_zig_simd"]:
            zig_mean, zig_med, zig_min, zig_p99, _ = timed(
                lambda: cosine_similarity(a, b),
                iterations=ITERATIONS,
            )
            speedup = py_mean / zig_mean if zig_mean > 0 else 0
            fmt_row(
    ...
    py_mean, py_med, py_min, py_p99, _ = timed(
        lambda: [_py_cosine(query, v) for v in vectors],
        iterations=200,
    )

    if status["has_zig_simd"]:
        zig_mean, zig_med, zig_min, zig_p99, _ = timed(
            lambda: batch_cosine(query, vectors),
            iterations=200,
        )
        speedup = py_mean / zig_mean if zig_mean > 0 else 0
...
def bench_distance_matrix():
    ...
    for n_vecs in [20, 50, 100]:
        ...
        py_mean, py_med, py_min, py_p99, _ = timed(
            lambda v=vectors: _py_distance_matrix(v),
            iterations=iters,
        )

        if status["has_zig_simd"]:
            zig_mean, zig_med, zig_min, zig_p99, _ = timed(
                lambda v=vectors: pairwise_distance_matrix(v),
                iterations=iters,
            )
            speedup = py_mean / zig_mean if zig_mean > 0 else 0
            fmt_row(
                f"Zig SIMD  [{n_vecs}×128]",
                zig_mean,
                zig_med,
                zig_min,
                zig_p99,
...
def bench_rust_holographic():
    ...
    # Single encode
    mem = {
        "content": "Test memory for holographic encoding benchmark",
        "importance": 0.8,
    ...
    rust_mean, rust_med, rust_min, rust_p99, _ = timed(
        lambda: holographic_encode_single(mem),
        iterations=ITERATIONS,
    )
    ...
    rust_mean, rust_med, rust_min, rust_p99, _ = timed(
        lambda: holographic_encode_batch(batch),
        iterations=500,
    )
...
def bench_rust_minhash():
    ...
    fmt_row(
        f"Rust MinHash [{len(keyword_sets)} sets]",
        rust_mean,
        rust_med,
        rust_min,
        rust_p99,
...
def bench_rust_bm25():
    ...
    for i in range(500):
        ...
        docs.append(
            f"Document {i} about {topic}. This discusses {topic} in detail with implementation notes."
        )
    ...
    for q in queries:
        rust_mean, rust_med, rust_min, rust_p99, _ = timed(
            lambda query=q: search_query(query, 10),
            iterations=ITERATIONS,
        )
...
def print_summary():
    ...
    try:
        ...
        s = simd_status()
        checks.append(
            ("Zig SIMD (cosine)", s["has_zig_simd"], s.get("lane_width", "N/A"))
        )
    except Exception:
