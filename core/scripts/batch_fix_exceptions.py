...
def fix_file(filepath: Path, apply: bool = False) -> dict:
    ...
    for i in range(len(lines)):
        ...
        elif any(x in context for x in ["open(", "read(", "write(", ".read()", ".write()", "path(", "file("]):
            replacement = "OSError, FileNotFoundError, PermissionError"
        elif any(x in context for x in ["json.loads", "json.dumps", "json.load", "json.dump"]):
            replacement = "json.JSONDecodeError, TypeError"
            import_add = "import json"
        elif any(x in context for x in ["sqlite", "execute(", "cursor(", "commit(", "db.execute"]):
            replacement = "sqlite3.Error, sqlite3.OperationalError"
            import_add = "import sqlite3"
        elif any(x in context for x in ["requests.", "urllib", "http", "connect(", "socket"]):
            replacement = "ConnectionError, TimeoutError, OSError"
        
        if replacement:
            # Replace the exception type
            new_line = re.sub(r'except Exception(\s+as\s+e)?:', f'except {replacement}\\1:', line)
            lines[i] = new_line
...
def main():
    ...
    print(f"{'Would fix' if args.dry_run else 'Fixed'} {total_changes} patterns in {len(results)} files\n")
