...
> **Version 21.0.0** — 448 callable tools (28 Gana STABLE meta-tools + 420 OPTIONAL dispatch tools)

## Tool Contract & Stability

Tools are classified by **stability tier**:
- **STABLE** (28 tools): The 28 Gana meta-tools — guaranteed public API, safe to rely on across releases
- **OPTIONAL** (420 tools): Available but may evolve — check `stability` field in tool definitions
- **EXPERIMENTAL** (0 tools currently): Preview/Labs surface — may change without notice

Query `surface_counts` tool for live breakdown: `call_tool("surface_counts")`
