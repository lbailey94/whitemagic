...
def test_solver_basic():
    ...
    start = time.time()
    result = call_tool(
        "solve_optimization", nodes=nodes, edges=edges, scores=scores, budget=budget
    )
    ...
    if result.get("status") == "success":
        ...
        if "Frontend" in selected and (
            "API" not in selected or "Infra" not in selected
        ):
...
def test_solver_dharmic():
    ...
    if result.get("status") == "success":
        ...
        if len(selected) == 3:
            ...
            console.print(
                f"[yellow]⚠️  Balance: {len(selected)}/6 nodes. (Optimal for scoring vs constraints).[/]"
            )
    else:
        console.print(f"[red]Failed: {result.get('message')}[/]")
