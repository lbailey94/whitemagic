...
class TestCascadeTriggers(unittest.TestCase):
    ...
    def test_get_history(self):
        ...
        for i in range(5):
            self.bus.emit(
                ResonanceEvent(source=f"src{i}", event_type=EventType.JOY_TRIGGERED)
            )
...
class TestCycleDetection(unittest.TestCase):
    ...
    def test_cyclic_table_blocks_cascades(self):
        ...
        bus.emit(
            ResonanceEvent(source="test", event_type=ET.JOY_TRIGGERED, confidence=0.8)
        )
...
class TestPyO3Acceleration(unittest.TestCase):
    ...
    def test_pyo3_backend_used_when_available(self):
        ...
        bus.emit(
            ResonanceEvent(source="test", event_type=all_events[0], confidence=0.8)
        )
    ...
    def test_pyo3_cascaded_events_correct(self):
        ...
        bus.emit(
            ResonanceEvent(source="test", event_type=ET.BEAUTY_DETECTED, confidence=0.5)
        )
        ...
        cascaded = [
            e for e in history if e.event_type in (ET.JOY_TRIGGERED, ET.PLAY_INITIATED)
        ]
        for e in cascaded:
...
class TestPolyglotBridges(unittest.TestCase):
    ...
    def test_koka_garden_resonance_returns_none_when_unavailable(self):
        ...
        result = bus._try_koka_garden_resonance(
            ResonanceEvent(
                source="test", event_type=self.EventType.JOY_TRIGGERED, confidence=0.8
            )
        )
