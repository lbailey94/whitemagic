        ...
        with memory_backend.pool.connection() as conn:
            ...
            row = conn.execute(
                """SELECT COALESCE(direction, 'undirected') as direction,
                          COALESCE(relation_type, 'associated_with') as relation_type,
                          COALESCE(edge_type, 'semantic') as edge_type
                   FROM associations WHERE source_id = 'old_a'"""
            ).fetchone()

        # Should get defaults via COALESCE
        assert row[0] == "undirected"
        assert row[1] == "associated_with"
        assert row[2] == "semantic"
