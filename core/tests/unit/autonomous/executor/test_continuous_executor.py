...
def executor(mock_config):
    with (
        patch("whitemagic.autonomous.executor.assessor.ProgressAssessor"),
        patch("whitemagic.autonomous.executor.limits.ResourceLimits"),
    ):
        # Mock what we need without importing
        with patch(
            "whitemagic.autonomous.executor.continuous_executor.ContinuousExecutor.log"
        ):
            ...
            ex.generator.form_strategy = AsyncMock(
                return_value={"focus": "testing", "goal": "Test everything"}
            )
            ex.generator.generate_strategy = AsyncMock(
                return_value={"focus": "testing"}
            )
            ex.generator.generate_objectives = AsyncMock(
                return_value=[Objective(description="Generated 1", id="gen1")]
            )
            return ex
...
async def test_execute_objective_success(executor):
    ...
    with patch.object(executor, "parse_objective", return_value=[mock_action]):
        with patch.object(executor, "run_command", new_callable=AsyncMock) as mock_run:
            with patch.object(
                executor.assessor, "assess", new_callable=AsyncMock
            ) as mock_assess:
...
async def test_run_continuous_empty_queue(executor):
    ...
    with patch.object(
        executor, "self_direct", new_callable=AsyncMock
    ) as mock_self_direct:
        with patch.object(
...
async def test_run_continuous_batch_execution(executor):
    ...
    with patch.object(
        ...
        with patch.object(
            executor, "execute_next", new_callable=AsyncMock
        ) as mock_exec_next:
            executor.limits.should_stop.side_effect = [
                (False, None),
                (True, "Stop for test"),
...
async def test_execute_batch_async(executor):
    ...
    with patch.object(
        executor, "execute_objective", new_callable=AsyncMock
    ) as mock_exec:
