# ruff: noqa: BLE001
"""Tests for the self-initiation queue (formerly self-prompting)."""
...
class TestSelfPrompting:
    ...
    def test_register_handler(self):
        ...
        # Pre-approve dharma gate since governor may not be available in tests
        items = _load_queue()
        for item in items:
            if item.task == "custom task" and item.status == "pending":
                item.dharma_approved = True
        from whitemagic.core.consciousness.self_initiation import _save_queue
        _save_queue(items)
        process_queue(limit=10)
        assert "custom task" in called

    def test_work_item_from_dict(self):
        from whitemagic.core.consciousness.self_initiation import WorkItem
    ...
    def test_max_attempts(self):
        ...
        items = _load_queue()
        for item in items:
            if item.task == "failing task" and item.status == "pending":
                item.dharma_approved = True
        _save_queue(items)
        # Process multiple times to exhaust attempts
        for _ in range(4):
            process_queue(limit=10)
            # Re-approve after each process (status reset to pending)
            items = _load_queue()
            for item in items:
                if item.task == "failing task" and item.status == "pending":
                    item.dharma_approved = True
