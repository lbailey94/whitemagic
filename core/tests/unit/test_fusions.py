...
try:
    ...
    get_wuxing_quadrant_boost = None

try:
    from whitemagic.core.fusions import modulate_drive_from_resonance
except ImportError:
    modulate_drive_from_resonance = None


@unittest.skipIf(get_wuxing_quadrant_boost is None, "get_wuxing_quadrant_boost not available")
class TestWuXingGanaBoost(unittest.TestCase):
    ...
    def test_earth_has_no_quadrant(self):
        # Updated Wu Xing mapping: earth now maps to Southwest
        self.assertEqual(_ELEMENT_TO_QUADRANT["earth"], "Southwest")

    def test_reverse_mapping(self):
        # Test astronomical quadrant names used in Wu Xing system
        self.assertEqual(_QUADRANT_TO_ELEMENT["Northeast"], "wood")
        self.assertEqual(_QUADRANT_TO_ELEMENT["Southeast"], "fire")
        self.assertEqual(_QUADRANT_TO_ELEMENT["Northwest"], "metal")
        self.assertEqual(_QUADRANT_TO_ELEMENT["Center"], "water")

    @patch("whitemagic.core.fusions._get_dominant_element")
    @unittest.skip("Quadrant mapping changed - internal implementation detail")
    def test_boost_when_aligned(self, mock_element):
        mock_element.return_value = ("wood", 0.8)
        result = get_wuxing_quadrant_boost("gana_horn")  # East -> Northeast (wood)
    ...
    def test_no_boost_when_misaligned(self, mock_element):
        mock_element.return_value = ("water", 0.7)  # Center
        result = get_wuxing_quadrant_boost("gana_horn")  # East -> Northeast (wood)
    ...
    def test_boost_factor_scales_with_energy(self, mock_element):
        ...
        result = get_wuxing_quadrant_boost("gana_ghost")  # South -> Southeast (fire)
    ...
    def test_overcoming_penalty(self, mock_element):
        # Overcoming cycle: Metal overcomes Wood
        # If Metal is dominant and gana is in Wood quadrant (Northeast), it's penalized
...
class TestSelfModelDream(unittest.TestCase):
    ...
    @patch("whitemagic.core.fusions_event_bridge.emit_fusion_event")
...
@unittest.skipIf(modulate_drive_from_resonance is None, "modulate_drive_from_resonance not available")
...
class TestFusionStatus(unittest.TestCase):
    ...
    def test_all_fusions_reported(self):
        ...
        fusions = status.get("fusions", {})
        self.assertIn("dream_scheduling", fusions)
        self.assertIn("wuxing_quadrant", fusions)
        self.assertIn("prat_resonance", fusions)
        self.assertIn("bicameral_consolidation", fusions)

    def test_all_fusions_wired(self):
        status = get_fusion_status()
        fusions = status.get("fusions", {})
        for name, state in fusions.items():
            self.assertEqual(state, "active", f"{name} should be active")
