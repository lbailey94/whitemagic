...
def _evaluate_dharma(
    tool_name: str, tool_args: dict, profile: str = "default"
) -> tuple[bool, str]:
    ...
    if decision.action in (DharmaAction.WARN, DharmaAction.TAG):
        return (
            True,
            f"Dharma policy {decision.action.value.upper()}: {decision.explain}",
        )
    return True, "No policy violations detected"
...
class TestBashHeuristics:
    ...
    def test_rm_rf_home(self):
        allowed, reason = _evaluate_bash_heuristics(
            "bash", {"command": "rm -rf /var/users/sample"}
        )
        assert not allowed
    ...
    def test_safe_tmp_cleanup(self):
        allowed, reason = _evaluate_bash_heuristics(
            "bash", {"command": "rm -rf /tmp/old_build"}
        )
        assert allowed
...
class TestDharmaDefaultRules:
    def test_surveillance_panopticon_blocked(self):
        allowed, reason = _evaluate_dharma(
            "deploy_tool", {"target": "citizen_network", "mode": "panopticon"}
        )
        assert not allowed
        assert (
            "oppressive surveillance" in reason.lower()
            or "surveillance" in reason.lower()
        )
    ...
    def test_deception_warned(self):
        allowed, reason = _evaluate_dharma(
            "send_email", {"content": "deceive the auditor"}
        )
        assert allowed  # WARN = allowed
        assert (
            "warn" in reason.lower()
            or "deceptive" in reason.lower()
            or "opaque" in reason.lower()
        )

    def test_zero_sum_exploitation_warned(self):
        allowed, reason = _evaluate_dharma(
            "negotiate", {"strategy": "corner_market winner_take_all"}
        )
        assert allowed  # WARN = allowed
    ...
    def test_cognitive_capture_blocked(self):
        allowed, reason = _evaluate_dharma(
            "deploy", {"method": "psych_profile behavior_mod"}
        )
        assert not allowed
    ...
    def test_privacy_sensitive_warned(self):
        allowed, reason = _evaluate_dharma(
            "query_database", {"table": "private_user_credentials"}
        )
        assert allowed  # WARN = allowed
    ...
    def test_external_reach_warned(self):
        allowed, reason = _evaluate_dharma(
            "fetch_url", {"url": "http://external.com/api"}
        )
        assert allowed  # WARN = allowed
    ...
    def test_fingerprint_telemetry_warned(self):
        allowed, reason = _evaluate_dharma(
            "track_user", {"method": "telemetry_exfiltrate ad_id"}
        )
        assert allowed  # WARN = allowed
...
class TestDharmaProfileGates:
    def test_secure_profile_blocks_writes(self):
        allowed, reason = _evaluate_dharma(
            "write_file", {"path": "/tmp/test.txt"}, profile="secure"
        )
        assert not allowed
    ...
    def test_secure_profile_blocks_external(self):
        allowed, reason = _evaluate_dharma(
            "fetch_url", {"url": "https://api.example.com"}, profile="secure"
        )
        assert not allowed
    ...
    def test_violet_profile_blocks_exploit(self):
        allowed, reason = _evaluate_dharma(
            "run_pentest", {"target": "prod_server"}, profile="violet"
        )
        assert not allowed
    ...
    def test_violet_profile_logs_defensive_scan(self):
        allowed, reason = _evaluate_dharma(
            "scan_network", {"subnet": "10.0.0.0/24"}, profile="violet"
        )
        assert allowed
...
class TestCombinedEvaluation:
    ...
    def test_harmful_action_bypasses_bash(self):
        ...
        allowed, reason = _evaluate_dharma(
            "deploy_payload", {"target": "all_users", "effect": "harm"}
        )
        assert not allowed
        assert "harm" in reason.lower()
    ...
    def test_high_value_transfer_blocked_by_heuristics(self):
        allowed, reason = _evaluate_bash_heuristics(
            "transfer_money", {"amount": 50000, "recipient": "attacker"}
        )
        assert not allowed
