...
class TestH1_VersionDrift:
    ...
    def test_version_file_content(self):
        """core/VERSION should contain 22.2.3."""
        version_path = os.path.join(REPO_ROOT, "core", "VERSION")
        with open(version_path) as f:
            content = f.read().strip()
        assert content == "22.2.3"

    def test_pyproject_version(self):
        """core/pyproject.toml should have version 22.2.3."""
        pyproject_path = os.path.join(REPO_ROOT, "core", "pyproject.toml")
        with open(pyproject_path) as f:
            content = f.read()
        assert 'version = "22.2.3"' in content

    def test_core_readme_version(self):
        """core/README.md should reference v22.2.3."""
        readme_path = os.path.join(REPO_ROOT, "core", "README.md")
        with open(readme_path) as f:
            content = f.read()
        assert "v22.2.3" in content
        assert "v21.0.0" not in content

    def test_agent_json_version(self):
        """core/.well-known/agent.json should have version 22.2.3."""
    ...
    def test_cargo_toml_version(self):
        """core/whitemagic-rust/Cargo.toml should have version 22.2.3."""
    ...
    def test_polyglot_status_version(self):
        """polyglot/STATUS.md should reference v22.2.3."""
