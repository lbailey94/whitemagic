...
class TestYieldCurve:
    ...
    def test_compounding(self):
        curve = YieldCurve(improvement_id="h1", yield_type=YieldType.COMPOUNDING, v0=1.0, r=0.1)
        v0 = curve.value_at(0)
        v10 = curve.value_at(10)
        assert abs(v0 - 1.0) < 1e-6
    ...
    def test_appreciating(self):
        curve = YieldCurve(improvement_id="h1", yield_type=YieldType.APPRECIATING, v0=1.0, tau=10.0)
        v0 = curve.value_at(0)
        v10 = curve.value_at(10)
        assert v10 > v0  # Appreciating

    def test_transient(self):
        curve = YieldCurve(improvement_id="h1", yield_type=YieldType.TRANSIENT, v0=1.0, lambda_=0.1, tau=5.0)
        v0 = curve.value_at(0)
        v5 = curve.value_at(5)
    ...
    def test_duration(self):
        decaying = YieldCurve(improvement_id="h1", yield_type=YieldType.DECAYING, lambda_=0.1)
        assert abs(decaying.duration() - 10.0) < 1e-6  # 1/λ

        compounding = YieldCurve(improvement_id="h2", yield_type=YieldType.COMPOUNDING, r=0.05)
...
class TestYieldPortfolio:
    ...
    def test_portfolio_duration(self):
        portfolio = YieldPortfolio()
        portfolio.add_curve(YieldCurve(improvement_id="h1", yield_type=YieldType.DECAYING, v0=1.0, lambda_=0.1))
        portfolio.add_curve(YieldCurve(improvement_id="h2", yield_type=YieldType.COMPOUNDING, v0=1.0, r=0.2))
        duration = portfolio.portfolio_duration()
        assert duration > 0

    def test_select_by_horizon(self):
        portfolio = YieldPortfolio()
        portfolio.add_curve(YieldCurve(improvement_id="h1", yield_type=YieldType.DECAYING, v0=1.0, lambda_=0.5))
        portfolio.add_curve(YieldCurve(improvement_id="h2", yield_type=YieldType.COMPOUNDING, v0=1.0, r=0.1))
    ...
    def test_stats(self):
        portfolio = YieldPortfolio()
        portfolio.add_curve(YieldCurve(improvement_id="h1", yield_type=YieldType.DECAYING))
        portfolio.add_curve(YieldCurve(improvement_id="h2", yield_type=YieldType.COMPOUNDING))
