...
def validate_5d(num_samples: int = 50, k: int = 10):
    ...
    if not engine or not engine.available(include_cache=True):
        print(
            "❌ ERROR: Embedding engine (and cache) unavailable. Cannot establish semantic ground truth."
        )
        return

    if not engine.available():
        print(
            "⚠️  Warning: Embedding model not installed. Using cached embeddings for ground truth."
        )
    ...
    with db:
        all_ids = [
            row[0]
            for row in db.execute("SELECT memory_id FROM memory_embeddings").fetchall()
        ]
    ...
    with um.backend.pool.connection() as conn:
        coord_ids = {
            ...
            for row in conn.execute(
                "SELECT memory_id FROM holographic_coords"
            ).fetchall()
        }
    ...
    print(
        f"Created isolated validation index. Populating with {len(valid_ids)} points..."
    )
    ...
    with um.backend.pool.connection() as conn:
        ...
        rows = conn.execute(
            f"SELECT id, content, title, metadata, memory_type, created_at, importance, access_count, galactic_distance, emotional_valence, neuro_score FROM memories WHERE id IN ({placeholders})",
            valid_ids,
        ).fetchall()
        ...
        for r in rows:
            ...
            validation_index.add(
                mem_dict["id"], [coord.x, coord.y, coord.z, coord.w, coord.v]
            )
    ...
    for mid in sample_ids:
        ...
        semantic_ids = [
            hit["memory_id"] for hit in semantic_hits if hit["memory_id"] != mid
        ]
    ...
    if avg_overlap_5d >= 50:
        ...
        print(
            "❌ FAILURE: 0% overlap. 5D system is completely decoupled from semantics."
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--samples", type=int, default=50, help="Number of random memories to sample"
    )
    parser.add_argument(
        "--k", type=int, default=10, help="Number of neighbors to compare"
    )
