...
def benchmark():
    ...
    for i in range(batch_size):
        memories.append(
            {
                "id": f"batch_{i}",
                "content": "This is a test memory for benchmarking holographic encoding speed."
                * 10,
    ...
    print(
        f"Python (optimized): {py_time:.2f}ms ({batch_size / ((py_end - py_start) or 0.001):.0f}/sec)"
    )
    ...
    print(
        f"Mojo (via stdin): {mojo_time:.2f}ms ({batch_size / ((mojo_end - mojo_start) or 0.001):.0f}/sec)"
    )
